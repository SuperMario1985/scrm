<template>
    <div>
        <div>
            ...
            <div class="content-hd" style="display: flex;justify-content: space-between">
                <div>
                    <a-input style="width: 15%;" placeholder="客户昵称搜索"
                             @keydown.enter="search"
                             v-model.trim="name"></a-input>
                    <a-select
                        optionFilterProp="children"
                        style="width: 170px;margin-bottom: 20px;margin:0 10px;"
                        v-model="status">
                        <a-select-option :value="0">删除原因</a-select-option>
                        <a-select-option :value="2">带删除原因</a-select-option>
                        <a-select-option :value="1">无删除原因</a-select-option>
                    </a-select>
                    ...
                    <a-range-picker
                        ...
                        v-model="deleteDate"
                    />
                    <a-button @click="search" style="margin: 0 10px" type="primary">查找</a-button>
                ...
                <div>
                    <a-dropdown>
                        ...
                        <a-button type="primary">导出
                            <a-icon style="color: #FFFFFF;" type="down"/>
                        </a-button>
                    </a-dropdown>
                    <a-button type="primary" style="margin-left: 10px" @click="getAgentList">更新数据</a-button>
                </div>
            </div>
                ...
                <a-spin tip="加载中..." size="large" :spinning="isLoading">
                    <a-table :columns="columns" :dataSource="deleteList" :pagination="false"
                        ...
                        <div slot="tagName" slot-scope="text, record, index">
                            ...
                            <span v-if="record.perName.length == 0 && record.tagName.length > 5">
                      <a-popover>
                          <span slot="content">
                          ...
                          <span v-for="(item1,index1) in record.tagName">
                                <a-tag v-if="index1<5" style="margin-top: 5px;"
                                       color="blue">
												          {{ item1 }}
											            </a-tag>
                          </span>
                        <span>等{{ record.tagName.length }}个标签</span>
                      </a-popover>
                </span>
                            <span v-if="record.perName.length == 0 && record.tagName.length <=5">
                    	<a-tag v-for="item in record.tagName" style="margin-top: 5px;"
                        ...
                        <div slot="create_time" slot-scope="text, record, index">
                            ...
                            <div v-if="record.del_type==2" style="color:red">删除原因：客户已将员工删除</div>
                        </div>
                        <div slot="tag_name2">
                            标签
