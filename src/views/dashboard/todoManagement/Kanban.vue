<template>
	<div style="width: 100%;height:100%;">
		<div style="height:100%;">
						...
						<div class="part" v-for="(item,index) in list" :id="'part'+ item.id" v-show="list.length > 0">
							<a-spin tip="Loading..." :spinning="isSpinning[index]">
								...
								<a-row
									...
									<a-empty v-if="item.members.length == 0" style="margin-top: 100px;"/>
									<div class="part-body-card" v-for="part in item.members"
									     v-if="item.members.length > 0"
									     :ref="'part_body_card_' + part.id" :id="part.id">
										<div style="padding: 10px 10px 0;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;word-break: break-all;font-weight: 700;">
											{{part.title}}
										</div>
										<div style="overflow: hidden;padding: 10px 5px 10px 10px;"
										     @click="part.status == 1 ? '' : customDetail(part.id,part.end_time,part.task_id,part.external_userid,part.sea_id)">
											...
											<img src="../../../assets/useradvart.png" alt="" v-if="!part.avatar"
											     style="float: left;width: 47px;margin-right: 5px;height:47px;border-radius: 5px;">
											<div style="float: left;width: calc(100% - 52px);font-size:12px;">
												...
												<p style="margin-top: 5px;">
													<span style="overflow:hidden;white-space: nowrap;text-overflow: ellipsis;max-width: 89px;display: inline-block;border: 1px solid;border-radius: 4px;padding: 0 5px;"
										...
										<div style="padding: 0 10px;margin-bottom: 10px;"
											...
											<p style="overflow:hidden;white-space: nowrap;text-overflow: ellipsis;"
											   v-if="part.status == 2 || part.status == 3">
												项目开始时间：{{part.start_time}}</p>
											<p style="overflow:hidden;white-space: nowrap;text-overflow: ellipsis;"
											   v-if="part.status == 2 || part.status == 3">
												预计截止时间：{{part.end_time}}</p>
											<!--												<p style="overflow:hidden;white-space: nowrap;text-overflow: ellipsis;"-->
											<!--												   v-if="part.status == 2 || part.status == 3">-->
											...
											<p style="white-space: normal;"
											   v-if="part.status == 1">
												项目规则：<span v-if="part.type == 1">手动启动，</span><span
													v-if="part.type == 2">立即启动，</span><span v-if="part.type == 3">在{{part.start_days}}天后启动，</span>
												需要{{part.days}}天内完成
											</p>
										</div>
										<div style="background: #F3F3F3;padding: 7px 10px;overflow:hidden;font-size: 12px;">
											...
											<a-button style="float: right;height: 24px;" type="primary"
											          ...
											          v-if="part.status == 1 && part.can_edit == 1">开始
											</a-button>
											<a-button style="float: right;height: 24px;" type="primary"
											          @click="addRecord(part.id,item.id,part.per,part.nickname,part.end_time,part.task_id,part.external_userid,part.sea_id)"
											          v-if="part.status == 2 && part.can_edit == 1">项目跟进
											</a-button>
										</div>
									</div>
								</a-row>
				...
				<div style="height: calc(100% - 51px);overflow:auto;">
					...
					<div style="padding: 20px;">
						...
						<a-input-number v-model="close_rate"
						                style="width: 165px;margin-right: 5px;" placeholder="请输入项目进度"/>
						%
					</div>
					<div class="textArea">
						...
						<a-textarea
								...
								v-if="showTextArea"
						/>
						<div style="text-align: right;margin-top: 10px;">{{followMsg.length}}/200
						</div>
					</div>
				</div>
				<div class="footer">
			...
			>
				<div style="height: calc(100% - 51px);overflow:auto;">
					...
					<div style="padding: 20px 20px 0;"
					     v-if="detailFinishTime.finish_time && detailFinishTime.finish_time != ''">
						实际完成：{{detailFinishTime.finish_time}}
						<span v-if="detailFinishTime.is_finish == 1">【按时完成】</span>
						<span v-if="detailFinishTime.is_finish == 2" style="color: red;">【超时{{detailFinishTime.delay_days}}天】</span>
						<span v-if="detailFinishTime.is_finish == 3">【提前{{detailFinishTime.pre_days}}天】</span>
					</div>
					<div style="padding: 20px 20px 0;">
						<div v-for="item in detailData">
							<p style="font-size: 16px;font-weight: 700;">{{item.date}}</p>
							<a-steps progress-dot direction="vertical" :current="item.data.length-1">
								<a-step :description="step.per_desc" v-for="step in item.data">
									<template slot="title">
										<span style="font-weight: 700;">{{step.time}}</span>
										<span :style="{'color':step.status_color}" style="margin-left: 10px;">{{step.status_title}}</span>
										<a-tag color="green"
		...
		<!--部门选择框-->
		<chooseDepartment ref="user" :id="corpId1" :show="showModalDepartment" :chooseNum="chooseNum"
...
<script>
	...
	export default {
		...
		methods   : {
			...
			//添加跟进
			addRecord (cid, id, close_rate, name, end_time, task_id, external_userid, sea_id) {
			...
			async getAccount () {
				...
				if (res.error != 0) {
					...
					that.$nextTick(() => {
						...
						that.$refs.scroll.style.height = 'calc(100% - ' + searchAreaHeight + 'px)'
					})
					if (that.cid != '') {
						that.$nextTick(() => {
