<template>
	<div class="app-center">
        ...
        <div class="app" :style="{ width: width }" v-for="item in appList">
          ...
          <div class="app-detail">
            ...
            <div style="margin-top: 12px; text-align: right">
              <a-button type="primary" @click="goUrl(url, item.type, item.name)"
                >进入管理
              </a-button>
            </div>
          </div>
		...
		<a-modal v-model="showStoreVisible" width="720px" :title="title" @cancel="cancelAddStore">
			...
			<a-form>
				<a-form-item label="店铺类型" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
					<a-select
						showSearch
						:allowClear="false"
					...
					>
						...
						<a-select-option :value="5">{{ shopName }}</a-select-option>
					</a-select>
				</a-form-item>
				<div style="color: #FF562D;margin-bottom: 10px;margin-left: 16.66%;"
				     v-if="storeType == 1 || storeType == 5">
					绑定后，可获取到该店铺的交易订单和会员信息。方便员工在与客户聊天时，可通过企业微信侧边栏中快速查看到该客户的历史订单记录与会员情况。
				</div>
				<a-form-item v-if="storeType == 5" label="店铺名称" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
					<a-input
						allowClear
						placeholder="请输入店铺名称"
				...
				<a-form-item v-if="storeType == 1 || storeType == 5" :label="storeType == 5?'AppKey':'店铺标识'"
					...
					<a-input
						...
						style="width: 300px;margin-right:15px;"
						v-model="token"
						:disabled="storeType == 5?true:false"
					/>
					<a style="margin-left: 10px;vertical-align: middle;" v-if="storeType == 5 && token"
					   @click="copyText(token)">复制</a>
				</a-form-item>
				<a-form-item v-if="storeType == 5" label="店铺秘钥" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
					<a-input
						placeholder="请输入店铺秘钥"
						style="width: 300px;margin-right:15px;"
						v-model="storeSetConfig.shop_api_secret"
						:disabled="true"
					/>
					<a style="margin-left: 10px;vertical-align: middle;" v-if="storeSetConfig.shop_api_secret"
					   @click="copyText(storeSetConfig.shop_api_secret)">复制</a>
				</a-form-item>
				<a-form-item v-if="storeType == 5" label="订阅地址" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
					<a-input
						placeholder="请输入订阅地址"
						style="width: 300px;margin-right:15px;"
						v-model="storeSetConfig.push_url"
						:disabled="true"
					/>
					<a style="margin-left: 10px;vertical-align: middle;" v-if="storeSetConfig.push_url"
					   @click="copyText(storeSetConfig.push_url)">复制</a>
				</a-form-item>
				...
				<div class="content-msg" v-if="storeType == 5">
					<p style="margin-bottom: 2px; line-height: 25px;">
						请前往{{ shopName }}，应用营销》软件中心》SCRM 填写推送地址和店铺标识，请确保标识一一对应，否则可能造成数据错乱。
					</p>
					<p style="margin-bottom: 0px; line-height: 25px;">
						支付完成就会推订单，这些订单优先根据企微和公众号的微信身份对应，其次按照下单人手机号来对应。
					</p>
				</div>
				<a-form-item v-if="storeType == 1" label="开启行业集成" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
				...
				<a-form-item v-if="storeType == 3 || storeType == 4" label="店铺名称" :label-col="{ span: 5 }"
				             :wrapper-col="{ span: 19 }">
					<a-input
						allowClear
						placeholder="请输入店铺名称"
				...
				<a-form-item v-if="storeType == 5" label="历史订单拉取地址" :label-col="{ span: 5 }"
				             :wrapper-col="{ span: 19 }">
					<a-input
						allowClear
						placeholder="请输入历史订单拉取地址"
						style="width: 300px;margin-right:15px;"
						v-model="orderUrl"
					/>
				</a-form-item>
				<a-form-item v-if="storeType == 5 && 0" label="AppKey" :label-col="{ span: 5 }"
				             :wrapper-col="{ span: 19 }">
					<a-input
						allowClear
				...
				<a-form-item v-if="storeType == 5 && 0" label="密钥" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
					<a-input
						allowClear
						placeholder="请输入密钥"
						style="width: 300px;margin-right:15px;"
				...
				<div class="content-msg" v-if="storeType == 5">
					<p style="margin-bottom: 2px;line-height: 25px;">
						请前往{{ shopName }}，应用营销》软件中心》SCRM 获取拉取地址。并填写上面的AppKey和店铺密钥。若scrm已创建门店，请填写门店ID信息。
					</p>
					<p style="margin-bottom: 2px;line-height: 25px;">
						填写完成后请前往
						<router-link to="/thirdPartyStore/order?type=5">【订单管理】</router-link>
						页面点击拉取历史订单，拉取过来的订单都是已支付订单，根据下单人手机号来对应。
					</p>
				</div>
				<!--				<a-form-item v-if="storeType == 2" label="应用ID" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">-->
				<!--					<a-input-->
				<!--							allowClear-->
				<!--							placeholder="请输入应用ID"-->
				...
				<!--					/>-->
				<!--				</a-form-item>-->
				<!--				<a-form-item v-if="storeType == 2" label="应用secret" :label-col="{ span: 5 }"-->
				<!--				             :wrapper-col="{ span: 19 }">-->
				<!--					<a-input-->
				<!--							allowClear-->
				...
				<!--					/>-->
				<!--				</a-form-item>-->
				<!--				<a-form-item v-if="storeType == 2" label="应用code" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">-->
				<!--					<a-input-->
				<!--							allowClear-->
		...
		data () {
			return {
				...
				ourAppList      : [
					...
					// 	title1 : '管理电商订单',
					// 	title2 : '小猪电商',
					// 	title3 : '店铺开启后先拉取历史订单，然后会向订阅地址自动推送新增订单。',
					// 	type   : 5,
					// 	isHas  : 1
					// }
				],
				...
				storeSetConfig: {},//店铺设置配置
				storeName     : '',//店铺名称
				orderUrl      : '',//历史订单拉取地址
				thirdToken    : '',//第三方店铺标识（AppKey）
				secretKey     : '',//秘钥
			};
		},
		methods: {
          ...
          this.ourAppList[1].isHas = res.data.hasTm;
			...
			async handleAddStore () {
				...
				if (this.storeType == 1) {
					...
					if (this.orderUrl == "") {
						this.$message.warning('请填写历史订单拉取地址！')
						this.loading = false
						return false
					}
					// if (this.thirdToken == "") {
				...
				if (this.storeType == 5) {
					params = {
						...
						shop_api_key    : this.token,
						shop_api_secret : this.storeSetConfig.shop_api_secret,
						order_pull_url  : this.orderUrl,
						third_api_key   : this.token,
						third_api_secret: this.storeSetConfig.shop_api_secret,
					}
					if (this.storeSetConfig.id) {
						params.id = this.storeSetConfig.id
					}
					url = "shop-customer-order/save-order-set"
				} else {
					params = {
						uid          : localStorage.getItem("uid"),
						sign         : this.token,
					...
					url = "attachment/application-sign-add"
				}
			...
			async shopSet () {//店铺设置配置
				let that = this
				that.loading = true
				const {data: res} = await that.axios.post("shop-customer-order/get-order-set", {
					corp_id: localStorage.getItem('corpId') ? localStorage.getItem('corpId') : "",
				});
				if (res.error != 0) {
					that.loading = false
					...
					that.loading = false
					that.storeSetConfig = res.data;//店铺设置配置
					that.storeName = res.data.shop_name;//店铺名称
					that.token = res.data.shop_api_key;// 店铺标识
					that.orderUrl = res.data.order_pull_url;//历史订单拉取地址
					that.thirdToken = res.data.third_api_key;//第三方店铺标识(AppKey)
					that.secretKey = res.data.third_api_secret;//秘钥
				}
			},
			copyText (value) {//复制链接
				let that = this
