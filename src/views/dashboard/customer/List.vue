					...
					<a-layout-content>
						<div class="content-msg">
							<p style="margin-bottom: 2px;">
								1、向48小时内和公众号互动过的粉丝推送客服消息，不占用群发次数。但是为了避免对用户骚扰，禁止滥发消息，以免被封禁。建议单日对每位粉丝发送不超过5-10次。</p>
							<p style="margin-bottom: 2px;">
								互动行为：用户发送消息、点击自定义菜单、关注公众号、扫描二维码及支付成功、用户维权。</p>
							<p style="margin-bottom: 0;">2、只有在完成授权之后与公众号进行互动的粉丝可以接收到客服消息，因此初次使用时，建议在授权公众号后等待48小时再发送。</p>
						</div>
						<div class="content-hd">
							<a-col style="float:left;">
								<a-select v-model="status" style="width: 174px;margin-right:15px;"
								          @change="changeStatus">
								...
								<a-select showSearch optionFilterProp="children" allowClear
								          style="width: 174px;margin-right: 15px;" @change="changeMsgTitle"
								          :value="titleIndex"
								          placeholder="请选择消息名称" :filterOption="filterOption">
							...
							<a-col style="float:right;">
								<a-button type="primary" icon="plus" style="width: 150px;font-size:14px;" @click="add"
								          v-if="$store.state.authorizer_type != 'unauthorized' && verify_service" v-has="'customer-add'">
									新建客服消息
								</a-button>
						...
						<div class="content-bd">
							<a-spin tip="Loading..." size="large" :spinning="isLoading">
								<a-table :columns="columns" :dataSource="seniorList" :pagination="false"
									...
									<span slot="customerTitle">
						                  ...
						                  <a-tooltip placement="bottom">
							                    <template slot="title">
								                    <p style="margin-bottom: 0;line-height: 20px;">客服消息实际发送人数偏低：</p>
								                    <p style="margin-bottom: 0;line-height: 20px;">a、发送客服消息过多，超过每次互动能接收的上限（每个粉丝每次互动最多收到约10次）；</p>
								                    <p style="margin-bottom: 0;line-height: 20px;">b、群发时粉丝互动已超过48小时；c、部分互动粉丝已取消关注；</p>
								                    <p style="margin-bottom: 0;line-height: 20px;">d、公众号授权状态异常；</p>
								                    <p style="margin-bottom: 0;line-height: 20px;">e、群发时素材缺失（例如：群发图片已删除）；</p>
								                    <p style="margin-bottom: 0;line-height: 20px;">f、公众号在其他平台也设置了客服消息，当日累计发送次数达到50万上限。</p>
							                    </template>
									...
									<span slot="status" slot-scope="text, record">
                                        <a-tooltip>
                                            ...
                                            <span style="color: red;"
                                                  v-if="record.status=='发送失败'">{{record.status}}</span>
                                        </a-tooltip>
                                        <span v-if="record.status!='发送失败'">{{record.status}}</span>
                                    </span>
									<span slot="action" slot-scope="text, record,index">
                                        <template v-if="record.status=='未发送'">
                                            <!--<a-button style="margin-right: 10px;" @click="edit">修改</a-button>-->
                                            <a-button style="margin: 0px 5px 5px 0px;" @click="detail(index)"
                                                      v-has="'customer-info'">详情</a-button>
                                            <a-button style="margin: 0px 5px 5px 0px;" @click="deleteList(record.id)"
                                                      v-has="'customer-delete'">删除</a-button>
                                        </template>
								...
								<div style="width: 100%;position: absolute;margin-top: 20px;margin-bottom: 20px;"
								     v-show="total > 0" v-has="'customer-list'">
									...
									<div class="pagination" style="height: 32px;float: right;">
										<a-pagination :total="total" showSizeChanger :showQuickJumper="quickJumper"
										              ...
										              :pageSizeOptions="['15','30','50','100']" @change="changePage"
																...
																<template v-for="(art, artIndex) in artList">
																	<template v-if="artIndex == 0">
																		...
																		<img alt="example"
																		     ...
																		     slot="cover" style="object-fit: cover;"/>
																	</template>
																	<template v-if="artIndex != 0">
	...
	export default {
		...
		methods: {
			...
			async getSeniorList (page = 1, pageSize = this.pageSize) {
				...
				const {data: res} = await this.axios.post("kf-push-msg/list", {
				...
				if (res.error != 0) {
					...
					this.seniorList = res.data.kfData;
					this.isLoading = false;
			...
			async previewMsg (index) {
				const {data: res} = await this.axios.post("kf-push-msg/info", {
				...
				if (res.error != 0) {
					...
					if (res.data.msg_type == 1) {
						this.content = res.data.content.replace(/{nickname}/g, '<span>&nbsp;<span contenteditable="false" class="ant-tag ant-tag-orange">粉丝昵称</span>&nbsp;</span>').replace(/\n/g, "<br/>")
			...
			async delSenior (id) {
				const {data: res} = await this.axios.post("kf-push-msg/delete", {
					id: id
				});
				// console.log(res)
				if (res.error != 0) {
		...
		beforeRouteEnter (to, from, next) {
			...
			// 不！能！获取组件实例 `this`
			// 因为当守卫执行前，组件实例还没被创建
			if (from.path != '/customer/add' || typeof to.query.isRefresh != 'undefined' && to.query.isRefresh == '1') {
				next(vm => {
					//因为当钩子执行前，组件实例还没被创建
