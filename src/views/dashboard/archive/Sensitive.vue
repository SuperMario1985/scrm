    ...
    <div id="components-layout-demo-basic">
      ...
      <div class="content-msg">
        <p style="margin-bottom: 2px">
          1、当敏感词被不启用时，曾经所触发的内容依然在列表中可见。
        </p>
        <p style="margin-bottom: 2px">
          2、显示历史上开启会话存档的企业成员。若曾经设置过某员工违规提醒，现在对该员工关闭、不再开启会话存档或是该员工离职，那么其历史触发敏感词的监控内容，依然在列表中可见。
        </p>
        <p style="margin-bottom: 0">
          3、显示历史上设置过违规提醒的群聊。如曾经设置过某群聊违规提醒，现在该群聊不再设置，那么其历史触发敏感词的监控内容，依然在列表中可见。
        </p>
      </div>
      <div
        style="
      ...
      >
        <div style="padding: 0 20px">
          <span
            @click="changeTabs('1')"
            class="tabBtn"
            :class="{ activeBtn: tabKey == 1 }"
            v-has="'sensitive-control'"
            >敏感词监控</span
          >
          <span
            @click="changeTabs('2')"
            class="tabBtn"
            :class="{ activeBtn: tabKey == 2 }"
            v-has="'sensitive-words'"
            >敏感词词库</span
          >
        </div>
        <div
            ...
            <a-layout style="overflow: hidden" class="content">
              ...
              <a-layout-header>
                <div class="content-hd">
                  ...
                  <a-button
                    @click="showChatVisible"
                    style="width: 145px; margin-right: 5px"
                  >
                    <span style="text-align: center" v-if="chooseNum1 > 0"
                      >已选择{{ chooseNum1 }}个群</span
                    >
                ...
                <div
                  v-if="total1 > 0"
                  style="font-size: 14px; height: 40px; line-height: 40px"
                >
                  共有
              ...
              >
                ...
                <div class="page-spinning" style="margin-top: 10px">
                  <a-empty
                    v-if="sensitiveList.length == 0 && !isLoading"
                    ...
                    <div class="spin-content">
                      ...
                      >
                        ...
                        >
                          ...
                          <span v-if="item.is_chat != 1 && item.to_type == 1"
                            >成员</span
                          >
                          <span v-if="item.is_chat != 1 && item.to_type == 2"
                            >客户</span
                          >
                          <span style="font-weight: 700"
                            >【{{ item.to_name }}】</span
              ...
              <a-layout-header>
                <div class="content-hd">
                  <div style="float: left">
                    <a-input
                      style="width: 165px; margin: 0 5px"
                  ...
                  <div class="content-hd-right">
                    ...
                    >
                      <a-form-item
                        ...
                        :wrapper-col="{ span: 20 }"
                      >
                        <template slot="label"> 选择分组 </template>
                        <a-select
                          showSearch
                          optionFilterProp="children"
                          v-model="editGroupId"
                      ...
                      <a-form-item
                        ...
                        :wrapper-col="{ span: 20 }"
                      >
                        <template slot="label"> 敏感词名称 </template>
                        <p style="margin: 0px; font-size: 13px; color: #909399">
                          每个敏感词名称最多6个字。同时新建多个敏感词时，请用“空格”隔开
                        </p>
                        <a-input
                          placeholder="请输入敏感词（不得超过6个字符）"
                          v-model="inputValue"
                        />
                      </a-form-item>
                    </a-modal>
                ...
                <div class="content-bd" style="margin-top: 10px">
                  <a-spin tip="加载中..." size="large" :spinning="isLoading">
                    <div class="spin-content">
                      ...
                      >
                        <span slot="checkedBoxTitle"> </span>
                        <span
                          slot="checkedBox"
                        ...
                        <span slot="action" slot-scope="text, record">
                          <span
                            v-if="record.is_forbid != 1 && record.status == 2"
                          ...
                          >
                          <span
                            v-if="record.is_forbid != 1 && record.status == 1"
                          ...
                          >
                          <span
                            v-if="record.is_forbid != 1"
                            @click="showModal3(text, record)"
                            style="color: #01b065; cursor: pointer; margin: 5px"
                        ...
                        >
                          ...
                          >
                            <a-select-option value="0"
                              >选择当前页</a-select-option
                            >
                            <a-select-option value="1"
                              >选择所有</a-select-option
                            >
                          </a-select>
                          <a-button
                            class="btn-primary"
              ...
              >
                <div style="margin: 19px 0px">
                  ...
                  <span
                    ...
                    "
                    ><span style="color: red">{{ inputValue3 }}</span
                    >确定删除吗？</span
                  >
                </div>
              </a-modal>
...
export default {
  ...
  methods: {
    ...
    async getSensitiveList(page = 1, pageSize = this.pageSize1) {
      ...
      const { data: res } = await this.axios.post("limit-word/monitor", {
        ...
        corp_id: this.corpId,
        start_date: this.sendTime
          ? this.sendTime.length > 1
            ? moment(this.sendTime[0]).format("YYYY-MM-DD")
            : ""
          : "",
        end_date: this.sendTime
          ? this.sendTime.length > 1
            ? moment(this.sendTime[1]).format("YYYY-MM-DD")
            : ""
          : "",
        ...
        if (this.group_id == 0) {
          this.moveGroupId =
            this.groupList.length > 0
              ? this.groupList.find((g) => g.is_not_group == 1).id
              : [];
          this.editGroupId =
            this.groupList.length > 0
              ? this.groupList.find((g) => g.is_not_group == 1).id
    ...
    setCheckedBox(wordsList) {
      ...
      if (this.batchType == 0) {
        ...
        if (
          this.selectedRowKeys.length == this.wordKeys.length &&
    ...
    addKey(wordsList) {
      for (let i = 0; i < wordsList.length; i++) {
        let index = this.selectedRowKeys.findIndex(
          (id) => id === wordsList[i].key
        );
  ...
  created() {
    this.$store.dispatch("getCorp", (info) => {
      this.corpInfo = info;
      if (this.corpInfo.length > 0) {
        this.isAudit = this.corpInfo.find(
          (i) => i.corpid == this.corpId
        ).isAudit;
      }
      let that = this;
...
.wx-info::-webkit-scrollbar-thumb {
  /*滚动条里面小方块*/
  background: #ffffff;
}
