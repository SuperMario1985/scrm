	...
	<div class="list">
		<div id="components-layout-demo-basic" v-if="this.$store.state.wxArr.length !=0">
			<a-layout>
				...
				<a-layout-header>
					新建高级群发
					<router-link to="/senior/list" style="font-size: 16px;float: right;margin-right: 15px;">
				...
				<a-layout style class="fans-content">
					...
					<a-layout-sider>
						<div
								style="border:1px solid #E2D6D6;margin:auto;height:100%;width:100%;background:#FAFAFA;box-sizing: content-box;overflow-y: auto;"
						>
							<img
							...
							/>
							<!-- 图文素材 -->
							<div style="padding: 20px;" v-show="typeValue == 5">
								<!-- 选择后显示的 -->
							...
							<div style="padding: 20px;" v-show="typeValue == 4">
								<!-- 选择前显示的 -->
								<a-card hoverable style="width: 100%" v-show="closeShowModal2==false">
									<img
											alt="example"
									...
									<a-form>
										...
										<a-form-item label="消息类型" :label-col="{ span: 3 }" :wrapper-col="{ span: 15 }">
											...
											<div class="changeType">
												<div
														style="height: 110px;margin: auto;cursor: pointer;"
											...
											<div style="margin-right: 15px;float: right;" v-show="typeValue == 1">
												{{wordNum}}/{{wordLimit}}
											</div>
											<div style="padding: 0 15px;" v-show="typeValue == 1">
												点击插入：
...
<!--											<div class="changeType-text" v-show="typeValue == 1">-->
...
<!--											</div>-->
                      ...
                      <span style="color:blue;">{{peopleNum}}</span>人
											...
											<div v-show="fansValue == 1" class="changeFans">
												<a-form>
													<a-form-item
															...
															:wrapper-col="{ span: 20 }"
													>
														<a-radio-group @change="changeIsCustom" v-model="isCustom">
													...
													>
														<template v-for="(item,index) in tagsList">
															<a-checkable-tag
																	:checked="index == tag_checked"
																	@change="changeTags2(item.key,index)"
															>{{item.name}}
															</a-checkable-tag>
														</template>
													</a-form-item>
											...
											<div v-show="fansValue == 3" class="changeFans">
												<a-input
														placeholder="请输入粉丝openid" v-model="openids"
												/>
										...
										>
											...
											<div>
												<a-checkbox @change="changePhone" :defaultChecked="true">若有文章判定为转载，继续群发
												</a-checkbox>
											</div>
											...
											<div style="margin-top: 15px;">
												...
												<a-button style="margin-left: 15px;" @click="openPreview">发送预览
												</a-button>
											</div>
											<a-modal title="发送手机预览" v-model="visible" @cancel="handleCancel">
												...
												<a-input placeholder="请输入微信号以接收预览链接" v-model="username"/>
												<div class="phoneView-txt">预览功能仅用于公众号查看文章效果，不适用于公众传播，预览链接会在短期内失效</div>
											</a-modal>
										</a-form-item>
									</a-form>
								</div>
						...
						<!-- 选择公众号弹窗 -->
						<wxModal :show="showModal3" :callback="modalVisibleChange2" hasName="senior-send-add"></wxModal>
						<!-- 选择消息弹窗 -->
						<chooseMsg :show="showModal2" :type="typeValue2" :callback="modalVisibleChange"
						           :chooseId="chooseId" :author_id="author_id"></chooseMsg>
					</a-layout-content>
				</a-layout>
			</a-layout>
		</div>
...
<script>
	...
	export default {
		components: {chooseMsg, videoPlayer, wxModal, 'medium-editor': editor, VEmojiPicker},
		data () {
			...
			return {
				...
				service_type    : "", //0代表订阅号，1代表由历史老帐号升级后的订阅号，2代表服务号
				openids         : "", //指定粉丝多个;分割
				send_time       : '',//创建发送时间
		...
		methods: {
			...
			modalVisibleChange2 (event, wxNum, wxNickName) {
			...
			openShowModal2 (index, id) {
				...
				if (this.typeValue == 5) {
					this.typeValue2 = 1;
					this.author_id = localStorage.getItem('wxNum')
				} else {
			...
			changeTags2 (key, index) {
				this.tag_ids = key;
				this.tag_checked = index;
			...
			async createdMsg () {
				...
				const {data: res} = await this.axios.post("high-level-push-msg/add", {
					id         : this.createdId,
					wx_id:localStorage.getItem('wxNum'),
				...
				if (res.error == 0) {
					...
					this.isLoading = false
					this.$router.push("/senior/list?isRefresh=1")
				} else {
					this.$message.error(res.error_msg);
					this.isLoading = false
				}
			...
			handleOk2 (e) {
				// this.loading3 = true;
				if (this.username == '') {
					this.$message.error('请输入微信号以接收预览链接');
					// this.visible = false;
					this.username = ''
				} else {
					this.createdPreview();
		...
		created () {
			...
			this.$store.dispatch("getWx", info => {
				...
				_this.service_type = _this.wxInfo[_this.$store.state.wxNumIndex].service_type;
			});
