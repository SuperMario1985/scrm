	...
	<div style="position: absolute;left: 0;top: 0;right: 0;background: #FFF;">
		<a-spin tip="Loading..." size="large" :spinning="isLoading">
			<div class="view-title">{{title}}
				<router-link to="/channelCode/list" style="font-size: 16px;float: right;margin-right: 15px;">
			...
			<div style="padding: 20px;">
				<ul class="statistics-ul">
					<li style="background: #40A9FF;">
						...
						<p class="number">{{newDayCount}}</p>
						<p class="count">累计：{{newTotalCount}}</p>
					</li>
					<li style="background: #FFC71B;">
						...
						<p class="number">{{newDayDeleteCount}}</p>
						<p class="count">累计：{{newTotalDeleteCount}}</p>
					</li>
					<li style="background: #8E8AFF;">
						...
						<p class="number">{{newDayDeleteByUserCount}}</p>
						<p class="count">累计：{{newTotalDeleteByUserCount}}</p>
					</li>
					<li style="background: #FF688E;">
						...
						<p class="number">{{newDayIncreaseCount}}</p>
						<p class="count">累计：{{newTotalIncreaseCount}}</p>
					</li>
				</ul>
				<!--折线图-->
				<div class="all-card-calendar">
					...
					<div class="sec-card-caledar down-text"
						...
						<p style="margin-bottom: 0px;">
							这段时间里，共新增客户数
							<span>{{new_contact_cnt}}</span>
							人、被客户删除/拉黑
							<span>{{negative_feedback_cnt}}</span>
							人、员工删除客户
							<span>{{delete_cnt}}</span>
							人、净增人数
							<span>{{increase_cnt}}</span>
							人
				...
				<a-table :columns="columns" :dataSource="timeData" :pagination="false" :rowClassName="rowClassName">
					...
					<span slot="negative_feedback_cnt">
				        被客户删除/拉黑人数
						<a-tooltip placement="right">
							<template slot="title">
								<span>同一个员工重复被删除会重复计算</span>
...
<script>
	...
	const columns = [
		...
		},
		{
			...
			slots    : {title: "new_contact_cnt"},
			key      : "new_contact_cnt"
		},
		{
			dataIndex: "negative_feedback_cnt",
			slots    : {title: "negative_feedback_cnt"},
			key      : "negative_feedback_cnt"
		},
		{
			dataIndex: "delete_cnt",
			slots    : {title: "delete_cnt"},
			key      : "delete_cnt"
		},
		{
			dataIndex: "increase_cnt",
			slots    : {title: "increase_cnt"},
			key      : "increase_cnt"
		},
	];
	...
	export default {
		...
		data () {
			return {
				...
				newDayCount              : "0",
				newDayDeleteCount        : "0",
				newDayDeleteByUserCount  : "0",
				newDayIncreaseCount      : 0,
				newTotalCount            : "0",
				newTotalDeleteCount      : "0",
				newTotalDeleteByUserCount: "0",
		...
		methods: {
			...
			async getStatisticsSum () {
				...
				if (res.error != 0) {
					...
					this.newDayCount = res.data.newDayCount
					this.newDayDeleteCount = res.data.newDayDeleteCount
					this.newDayDeleteByUserCount = res.data.newDayDeleteByUserCount
					this.newDayIncreaseCount = res.data.newDayIncreaseCount
					this.newTotalCount = res.data.newTotalCount
					this.newTotalDeleteCount = res.data.newTotalDeleteCount
					this.newTotalDeleteByUserCount = res.data.newTotalDeleteByUserCount
					this.newTotalIncreaseCount = res.data.newTotalIncreaseCount
					this.isLoading = false
				}
			},
			async getStatisticsList () {
				...
				const {data: res} = await this.axios.post(
					"work-contact-way/work-contact-way-statistic",
					{
						way_id   : this.urlId,
						type     : this.timeType,
						s_date   : s_date,
				...
				if (res.error != 0) {
					...
					if (this.is_export == 0) {
						this.isLoading = false
						this.new_contact_cnt = res.data.code_new_contact_cnt
						this.negative_feedback_cnt = res.data.code_negative_feedback_cnt
						this.delete_cnt = res.data.code_delete_cnt
						this.increase_cnt = res.data.code_increase_cnt
						...
						this.timeDataList = res.data.data
