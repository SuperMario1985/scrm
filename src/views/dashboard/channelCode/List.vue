<template>
	<div class="list">
					...
					<a-layout-content>
						...
						<div class="content-hd">
							<a-col style="float: left;">
								<a-input style="width: 180px;" placeholder="搜索活码名称" @keyup.enter="selectTitle"
								...
								<a-button @click="showDepartmentList(0,3)" style="width: 210px;margin:0 5px;">
						...
						<div class="content-bd">
							<a-spin tip="Loading..." size="large" :spinning="isLoading">
								<a-table :columns="columns" :dataSource="wayList" :pagination="false"
									...
									<span slot="skip_verify" slot-scope="text, record, index">
										<template v-if="record.skip_verify == 0 && record.verify_all_day == 2">
											<p style="margin: 2px 0;">分时段自动通过：</p>
											<p style="margin: 2px 0 2px 5px;" v-for="item in record.verify_date">
												{{item.start_time}}~{{item.end_time}}
											</p>
										</template>
										<template
												v-if="record.skip_verify == 0 && record.verify_all_day == 1">自动通过</template>
										<template v-if="record.skip_verify == 1">需要验证</template>
									</span>
								...
								<div class="pagination"
								     ...
								     v-show="total > 0" v-has="'channel-list'">
									<div style="height: 32px;display: inline-block;line-height: 32px;margin-bottom: 10px;"
										...
										<a-popover trigger="hover">
											<template slot="content">
												<p class="batch-edit" @click="batchEdit(1)"
												   style="margin-bottom: 2px;text-align: center; cursor: pointer;">
													按周期编辑</p>
												<p class="batch-edit" @click="batchEdit(2)"
												   style="margin-bottom: 2px;text-align: center; cursor: pointer;">
													按特殊时期编辑</p>
											</template>
		...
		<!--    预览、编辑周成员、编辑时期成员 三合一    -->
		<!--    batchEditType false预览 true 编辑  specialType 0 预览 1 周成员编辑 2 时期成员编辑    -->
			...
			<a-form>
				...
				<a-form-item label="特殊时期" :label-col="{ span: 2 }" v-if="specialType == 2 || specialType == 0"
					...
					<a-switch :disabled="specialType == 0" :checked="specialTime == 1 ? true:false"
					          @change="changeSpecilTime"></a-switch>
					...
					<div v-if="specialTime == 1" class="content-msg">
						...
						<p style="margin-bottom: 2px;line-height: 20px;color: #FF562D">
					...
					<div v-if="specialTime == 1" v-for="(item, index) in specialDateList"
						...
						<div v-for="(time, timeIndex) in item.time" style="margin-left: 78px;">
							...
							<div v-if="editType == 2 && ((!batchEditType && time.party && time.party.length > 0) || batchEditType)">
				...
				<a-form-item v-if="specialType == 0" label="员工添加上限" :label-col="{ span: 3 }"
				             :wrapper-col="{ span: 21 }">
				...
				<a-form-item style="padding-bottom: 20px;"
				             v-if="specialType == 0 && is_limit == 2" label="备用员工"
				             :label-col="{ span: 3 }"
				             :wrapper-col="{ span: 21 }">
		...
		<a-modal v-model="limitVisible" title="修改成员上限" width="888px" @cancel="cancelChangeLimit">
			<template slot="footer">
				<a-button key="back" @click="cancelChangeLimit">取消</a-button>
				<a-button
						key="submit"
						type="primary"
						:loading="groupLoading"
			...
			<a-spin tip="Loading..." size="large" :spinning="limitLoading">
				<a-table
						...
						:dataSource="user_limit"
						:pagination="false"
						:rowClassName="rowClassName"
						style="max-width: 100%;"
				>
					<span slot="is_limit" slot-scope="text, record, index">
						<a-switch :checked="record.is_limit == 2 ? true : false"
						          @change="changeIsCeiling(index)"></a-switch>
					</span>
					<span slot="user_limit" slot-scope="text, record, index">
						<div v-if="record.is_limit == 2 && record.user_limit.length > 0">
							<div v-for="(limit,idx) in record.user_limit" style="margin: 5px 0;">
								<span style="width: 75px; display: inline-block;">{{limit.name}}</span>
								<a-input-number
										...
										:parse="limitDecimals"
										@input.native="e => inputLimitNum(e, index, idx)"
										v-model="limit.limit"></a-input-number>
							</div>
						</div>
						<span v-if="record.user_limit.length == 0 || record.is_limit == 1">--</span>
					</span>
					<span slot="spare_employee" slot-scope="text, record, index">
						<a-button v-if="record.is_limit == 2 && record.type == 2" style="margin-left: 10px;"
						          @click="showDepartmentList(index, 2)">
							选择企业成员
						</a-button>
						<div v-if="record.is_limit == 2 && record.spare_employee.length > 0 && record.type == 2">
<!--							<a-tooltip placement="left">-->
							<!--								<template slot="title">-->
							<!--									<span v-for="item in record.spare_employee">{{item.title}} </span>-->
							<!--								</template>-->
							<!--								<span style="display: inline-block; max-width: 170px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;vertical-align: middle;margin-left: 10px;">-->
							<!--									<span v-for="item in record.spare_employee">{{item.title}} </span>-->
							<!--								</span>-->
							<!--							</a-tooltip>-->
							<a-tooltip placement="left">
								<template slot="title">
									<div>
										<span v-for="user in record.spare_employee"> <template
												v-if="user.scopedSlots.title == 'custom'"> {{user.title}} </template></span>
									</div>
								</template>
								<span v-if="record.spare_employee.length > 0"
								      style="display: inline-block; max-width: 80px;overflow: hidden; text-overflow: ellipsis; white-space: nowrap;vertical-align: middle;margin: 0 10px;">
											<span v-for="user in record.spare_employee"> <template
													v-if="user.scopedSlots.title == 'custom'"> {{user.title}} </template></span>
										</span>
							</a-tooltip>
						</div>
						<chooseStaffSelect :ref="'staffSelect3'+ index"
						                   style="width: 160px;margin-left: 10px;display: inline-block;"
						                   v-show="record.type == 1 && record.is_limit == 2" :index="index"
						                   :index2="-1"
						                   :callback="selectStaffCallback"></chooseStaffSelect>
						<span v-if="record.is_limit != 2">--</span>
					</span>
				</a-table>
			</a-spin>
		</a-modal>
		<a-modal v-model="welComeVisible" title="编辑欢迎语" @ok="handleWelcome"
			...
			<a-form-item label="扫码推送" :label-col="{ span: 3 }"
			             :wrapper-col="{ span: 21 }">
				<a-switch :checked="pushValue" @change="changeStatus"/>
			...
			<a-tabs tabPosition="left" style="margin-top: 20px;" default-active-key="1"
				...
				<a-tab-pane key="1" tab="通用欢迎语">
					<a-form-item style="padding-bottom: 20px;" v-if="pushValue"
					             label="" :label-col="{ span: 3 }"
					             :wrapper-col="{ span: 21 }">
						<div class="wrap left-sider-content">
							<img src="../../../assets/wxHeader.png" alt
							     style="width:250px;border-bottom:1px solid #E2D6D6;"/>
							<div class="msg_content_body">
								...
								<div v-if="img && pushValue" class="mt">
									<a-avatar
											src="https://zos.alipayobjects.com/rmsportal/ODTLcjxAfvqbxHnVXCYX.png"
											:size="36"
											style="margin-right: 10px;float: left;object-fit: cover;"/>
				...
				<a-tab-pane key="2" tab="周期欢迎语" force-render>
					<a-form-item style="padding-bottom: 20px;" v-if="pushValue"
					             label="" :label-col="{ span: 3 }"
					             :wrapper-col="{ span: 21 }">
						<a-switch :checked="is_welcome_week == 2"
						          @change="changeIsWelcomeWeek"></a-switch>
						<a-tabs v-if="is_welcome_week == 2"
						        v-model="weekTabKey"
						        size="large"
						        tabPosition="top"
						        style="width: 640px; margin-top: 5px;background-color: #FFF;padding-bottom: 20px;"
						        type="editable-card"
						        @edit="addWelcomeWeek">
							<a-tab-pane v-for="(week, index) in welcome_week_list" :key="index"
							            style="margin-left: 20px;height: 575px; overflow-y: auto;"
							            :tab="'欢迎语'+(index+1)">
								<div style="margin: 10px 0;">
									<label>选择周期：</label>
									<template v-for="w in weekList">
										<a-checkable-tag :checked="week.date.indexOf(w.key) > -1"
										                 style="cursor: pointer;"
										...
										<a-button
												...
												v-if="isDisabled(index, w.key)" :disabled="true">
											{{w.name}}
										</a-button>
									</template>
								</div>
								<div style="margin-left: 40px;" v-for="(item, idx) in week.time">
									<div style="margin-bottom: 10px;">
										<label style="margin-right: 10px;">时间段{{idx + 1}}：</label>
									...
									<div class="wrap left-sider-content">
										...
										<div class="msg_content_body">
											...
											<!--图片-->
											<div v-if="item.content.img && pushValue"
											     class="mt">
												<a-avatar
														src="https://zos.alipayobjects.com/rmsportal/ODTLcjxAfvqbxHnVXCYX.png"
														:size="36"
														style="margin-right: 10px;float: left;object-fit: cover;"/>
												<img :src="commonUrl+item.content.img" alt
												     style="width: 150px;border-radius: 4px;"/>
											</div>
											<!--网址-->
											<div v-if="item.content.modalUrlOk && pushValue"
											     class="mt">
												<a-avatar
														src="https://zos.alipayobjects.com/rmsportal/ODTLcjxAfvqbxHnVXCYX.png"
														:size="36"
												...
												<div class="item-info msg_content_txt">
													<p class="url-title">
														{{item.content.inputTitle}}</p>
													<div style="overflow: hidden;">
														<div class="url-text">
															{{item.content.digest}}
														</div>
														<img :src="commonUrl+item.content.msgUrl"
														     alt=""
														     style="float: right;width: 64px;height: 64px;object-fit: cover;">
													</div>
												</div>
											...
											<!--小程序-->
											<div v-if="item.content.modalAppletOk && pushValue"
											     class="mt">
												<a-avatar
														src="https://zos.alipayobjects.com/rmsportal/ODTLcjxAfvqbxHnVXCYX.png"
														:size="36"
												...
												<div class="item-info msg_content_txt">
													<p class="applet-title">
														{{item.content.appletInputTitle}}</p>
													<img :src="commonUrl+item.content.appletUrl"
													     alt=""
													     style="width: 100%;object-fit: cover;">
												</div>
											</div>
									...
									<div class="pull-right fl" style="margin-left: 30px;">
										...
										<!-- 网址 -->
										<div v-if="item.content.modalUrlOk"
										     style="margin: 10px 0;cursor: pointer;float:left;max-width: 200px;overflow: hidden;height: 40px;text-overflow: ellipsis;white-space: nowrap;"
										     @click="chooseUrl(2, 1, index, idx)">
											<a-icon type="link" style="margin-right: 7px;"/>
										...
										<!-- 小程序 -->
										<div v-if="item.content.modalAppletOk"
										     style="margin: 10px 0;cursor: pointer;float:left;max-width: 200px;overflow: hidden;height: 40px;text-overflow: ellipsis;white-space: nowrap;"
										     @click="chooseApplet(3, 1, index, idx)">
											<MyIcon type="icon-miniapp"></MyIcon>
								...
								<div style="margin-left: 40px;">
									<a-button @click="addWelcomeWeekTime(index)">添加时间段</a-button>
								</div>
							</a-tab-pane>
						</a-tabs>
					</a-form-item>
				...
				<a-tab-pane key="3" tab="特殊时期欢迎语">
					<a-form-item style="padding-bottom: 20px;" v-if="pushValue"
					             label="" :label-col="{ span: 3 }"
					             :wrapper-col="{ span: 21 }">
						<a-switch :checked="is_welcome_date == 2"
						          @change="changeIsWelcomeDate"></a-switch>
						<a-tabs
								v-model="dateTabKey"
								v-if="is_welcome_date == 2"
								size="large"
								tabPosition="top"
								style="width: 640px; margin-top: 5px;background-color: #FFF;padding-bottom: 20px;"
								type="editable-card"
								@edit="addWelcomeDate">
							<a-tab-pane v-for="(week, index) in welcome_date_list" :key="index"
							            style="margin-left: 20px;height: 575px; overflow-y: auto;"
							            :tab="'欢迎语' + (index + 1)">
								<div style="margin: 10px 0;">
								...
								<div style="margin-left: 40px;">
									<a-button @click="addWelcomeDateTime(index)">添加时间段</a-button>
								</div>
							</a-tab-pane>
						</a-tabs>
					</a-form-item>
		...
		<a-modal :visible="specialTimeVisible" :title="specialTimeTitle" @ok="handleSpecialTime"
			...
			<a-form-item v-if="specialTimeVisible" label="固定时段" :label-col="{ span: 3 }"
			             :wrapper-col="{ span: 21 }">
				<div>
					<div v-for="(item,index) in specialTimeList">
						...
						<a-tooltip placement="top" v-if="editType == 2">
							<template slot="title">
								<span v-for="item1 in item.userList">{{item1.title}} </span>
							</template>
						...
						<chooseStaffSelect ref="staffSelect2"
						                   style="width: 160px;margin-left: 10px;display: inline-block;"
						                   v-if="editType == 1" :index="index" :index2="0"
		...
		<a-modal :visible="showModalApplet" title="小程序" @ok="handleApplet"
			...
			<template v-if="miniproAddType == 1">
				...
				<a-form-item :label-col="{ span: 3 }"
					...
					<span
							style="color: blue;cursor: pointer;margin: 67px 0 0px 10px;float: left;"
							@click="choosePic(3, welcomeType, weekIndex, wTimeIndex)">重新上传</span>
				</a-form-item>
				<!-- 填写标题 -->
				<a-form-item :label-col="{ span: 3 }"
				             :wrapper-col="{ span: 21 }">
...
<script>
	...
	const limitUserColumns = [
		...
		},
		{
			...
			key        : "user_limit",
			scopedSlots: {customRender: 'user_limit'},
		},
		{
			title      : '备用员工',
			dataIndex  : "spare_employee",
			key        : "spare_employee",
			scopedSlots: {customRender: 'spare_employee'},
		}
	];
	const columns = [
		{
	...
	export default {
		...
		methods: {
			...
			showModal () {
				...
				this.$router.push('/channelCode/add?groupId=' + groupId)
			},
			//获取表格内容
			...
			async deleteList2 (id) {
				const {data: res} = await this.axios.post('work-contact-way/delete', {
					id: id
				})
			...
			async getCustomer (page = 1, pageSize = this.pageSize2) {
				const {data: res} = await this.axios.post('work-contact-way/get-custom', {
					way_id  : this.urlId,
					page    : page,
					pageSize: pageSize
				})
			...
			async getWayInfo (urlId) {
				const {data: res} = await this.axios.post('work-contact-way/get-info', {
					suite_id : 1,
					corp_id  : this.corpId,
					config_id: urlId,
				})
			...
			async batchEdit (type) {
				this.getUserLimit(type - 1)
				const {data: res} = await this.axios.post("work-contact-way/edit-all", {
				...
				if (res.error != 0) {
					...
					if (type == 1) {
						this.specialModalTitle = '编辑周期成员'
					} else {
						this.specialModalTitle = '编辑特殊时期成员'
					}
			...
			async getUserLimit (type) {
				this.limitLoading = true
				const {data: res} = await this.axios.post("work-contact-way/get-user-limit", {
					ids : this.selectedRowKeys,
					type: type
				});
				if (res.error != 0) {
					...
					this.user_limit = res.data
					this.userLimits = JSON.parse(JSON.stringify(res.data))
					this.limitLoading = false
					this.$nextTick(() => {
						for (let index = 0; index < this.user_limit.length; index++) {
							if (this.user_limit[index].type == 1) {
								this.$refs['staffSelect3' + index].userId = this.user_limit[index].spare_employee
							}
						}
					})
				}
			},
			changeIsCeiling (index) {
				if (this.user_limit[index].user_limit.length == 0) {
					this.$message.warning('因受官方限制，无法对动态部门的员工设置添加好友上限！')
					return false
				}
				this.user_limit[index].is_limit = this.user_limit[index].is_limit = this.user_limit[index].is_limit == 1 ? 2 : 1
			},
			// 数字输入框限制
			limitDecimals (value) {
				const reg = /[^\d]/g
			...
			inputLimitNum (e, index, idx) {
				if (e.data) {
					let re = /^[0-9]+$/
					console.log(this.user_limit[index].user_limit[idx].limit)
					if (!re.test(e.data) || this.user_limit[index].user_limit[idx].limit > 99999999) {
						this.user_limit[index].user_limit[idx].limit = parseInt(this.user_limit[index].user_limit[idx].limit.toString().substring(0, this.user_limit[index].user_limit[idx].limit.toString().length - 1)) || ''
						this.$forceUpdate()
					}
				}
			},
			async handleChangeSpecialTime () {
				...
				// for (let i = 0; i < this.user_limit.length; i++) {
				// 	if (this.user_limit[i].is_limit == 2) {
				// 		for (let leng = 0; leng < this.user_limit[i].user_limit.length; leng++) {
				// 			if (!this.user_limit[i].user_limit[leng].limit) {
				// 				this.$message.warning("员工上限不可为空！")
				// 				this.groupLoading = false
				// 				return false
				// 			}
				// 		}
				// 		if (this.user_limit[i].spare_employee.length == 0) {
				// 			this.$message.destroy()
			...
			async handleChangeLimit () {
				...
				for (let i = 0; i < this.user_limit.length; i++) {
					if (this.user_limit[i].is_limit == 2) {
						...
						if (this.user_limit[i].is_limit == 2 && this.user_limit[i].spare_employee.length == 0) {
							this.$message.destroy()
							this.$message.warning('请选择备用员工！')
							this.groupLoading = false
							return false
				...
				const {data: res} = await this.axios.post("work-contact-way/set-user-limit", {
					way_data: this.user_limit,
					ids     : this.selectedRowKeys
				});
				if (res.error != 0) {
			...
			getUniqueUserList () {
				...
				if (this.specialType == 1) {
					...
					for (let d of weekDay) {
						for (let week of this.specialWeekList[0][d]) {
							if (this.editType == 1) {
								if (week.userList) {
				...
				if (this.specialType == 2) {
					if (this.specialTime == 1) {
						for (let date of this.specialDateList) {
			...
			getLimitUser (userList) {
				let obj = {}
				for (let len = 0; len < this.userLimits.length; len++) {
					this.user_limit[len].user_limit = JSON.parse(JSON.stringify(this.userLimits[len].user_limit))
				}
				userList = userList.reduce((cur, next) => {
					obj[next.user_id] ? "" : obj[next.user_id] = true && cur.push(next);
					return cur;
				}, [])
				for (let k = 0; k < this.userLimits.length; k++) {
					for (let l = 0; l < userList.length; l++) {
						if (this.userLimits[k].user_limit.findIndex(u => u.user_id == userList[l].user_id) == -1) {
							this.user_limit[k].user_limit.push(userList[l])
						}
					}
				}
			...
			handleSpecialTime () {
				...
				for (let item of this.specialTimeList) {
					if (item.userList.length == 0 && item.party.length == 0 && this.editType == 2) {
						this.$message.warning("请选择成员或企业微信部门")
						this.addSpecialTimeLoading = false
						flag = true
						return false
					}
					if (item.userList.length == 0 && this.editType == 1) {
			...
			changeSpecilTime () {
				...
				// if(this.specialType != 1) {
				// 	this.user_limit = JSON.parse(JSON.stringify(this.userLimits))
				// } else {
				// 	this.getUniqueUserList()
				// }
			},
			...
			selectStaffCallback (event, id, index, index2) {
				if (event == "ok") {
					if (index2 == -1) {
						this.user_limit[index].spare_employee = id
					} else {
			...
			showDepartmentList (index, type, timeIndex) {
				if (type == 1) {
					...
					this.currentIndex = index
					let arr = []
					this.user_limit[index].spare_employee.map(x => {
						arr.push(x.id)
					})
					this.chooseDepartmentNum = this.user_limit[index].spare_employee.length
					this.departmentDisabled = 1
					this.$refs.user.rightIdList = JSON.parse(JSON.stringify(arr))
					this.$refs.user.rightList = JSON.parse(JSON.stringify(this.user_limit[index].spare_employee))
				} else if (type == 3) {
					this.currentType = type
					this.$refs.user.rightIdList = JSON.parse(JSON.stringify(this.user_ids))
					this.$refs.user.rightList = JSON.parse(JSON.stringify(this.user_arr))
				} else {
					this.currentIndex = index
					...
					this.chooseDepartmentNum = this.specialDateList[index].time[timeIndex].userList.length
					this.departmentDisabled = 0
					this.$refs.user.rightIdList = JSON.parse(JSON.stringify(arr))
			...
			modalVisibleChange3 (event, arr, userNum, departmentNum, list) {
				if (event == "ok") {
					if (this.currentType == 1) {
						...
						this.user_limit[this.currentIndex].spare_employee = []
						let flag = false
						...
						if (userNum + departmentNum > 0) {
							list.map(item => {
								this.user_limit[this.currentIndex].spare_employee.push(JSON.parse(JSON.stringify(item)))
							})
						}
			...
			async moveQrcode1 (id) {
				const {data: res} = await this.axios.post("work-contact-way/group-change", {
					uid     : localStorage.getItem('uid'),
					corp_id : this.corpId,
					group_id: this.group,
					way_id  : id
			...
			//编辑
			modifyWay (urlId) {
				this.$router.push({path: "/channelCode/add", query: {id: urlId}});
			}
			,
			...
			resetWelcome () {
				...
				this.disabledSync = 0
				this.materialSync = 0
				this.is_welcome_date = 1
				this.is_welcome_week = 1
			...
			checkedOut () {
				...
				let dataFlag = false
				if (this.pushValue) {
					if (this.textContent.trim().replace(/\n/g, '') == "" && !this.img && !this.modalUrlOk && !this.modalAppletOk) {
					...
					if (this.is_welcome_week == 2) {
						for (let index = 0; index < this.welcome_week_list.length; index++) {
							...
							for (let idx = 0; idx < week.time.length; idx++) {
								...
								// 周期欢迎语为空判断
								if (time.content.textContent.trim().replace(/\n/g, '') == "" && !time.content.img && !time.content.modalUrlOk && !time.content.modalAppletOk) {
			...
			async handleWelcome () {
				...
				const {data: res} = await this.axios.post("work-contact-way/edit-welcome", sendData);
				if (res.error != 0) {
					this.$message.error(res.error_msg);
				} else {
					this.$message.success("编辑成功")
			...
			changeIsWelcomeWeek () {
				this.is_welcome_week = this.is_welcome_week == 1 ? 2 : 1
			},
			...
			changeIsWelcomeDate () {
				this.is_welcome_date = this.is_welcome_date == 1 ? 2 : 1
			},
			// 添加时期欢迎语周期
		...
		beforeRouteEnter (to, from, next) {
			...
			// 不！能！获取组件实例 `this`
			// 因为当守卫执行前，组件实例还没被创建
			if (from.path == '/channelCode/add' && to.query.isRefresh == '1') {
				next(vm => {
					vm.selectedRowKeys = []
