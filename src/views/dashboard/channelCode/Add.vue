<template>
	<div class="list">
						...
						<a-layout-content>
							...
							<div class="content steps-content">
								...
								<div class="content-bd">
									<a-form>
										<a-form-item v-if="$store.state.corpArr.length > 1 && current == 0"
										             label="选择企业微信"
										             :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }">
											<template>
										...
										<a-form-item v-if="current == 0" label="选择分组" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }">
											<a-tree-select
													style="width: 225px"
										...
										<!-- 二维码名称 -->
										<a-form-item v-if="current == 0" label="活码名称" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }"
										...
										<!-- 是否需要验证 -->
										<a-form-item v-if="current == 0" label="自动添加好友" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }">
										...
										<!-- 客户标签 -->
										<a-form-item style="padding-bottom: 20px;" v-if="current == 0"
											...
											<corpChooseTag :key="1" :callback="modalVisibleChange4"
										...
										<a-form-item v-if="current == 1" label="类型" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }">
										...
										<!-- 排班 -->
										<a-form-item v-if="current == 1" label="企业成员" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }">
											<div>
												<a-button
														...
														<template v-if="info.type == 2">
															...
															<a-tooltip placement="top">
																...
																<span v-if="time.userList.length > 0"
																      style="display: inline-block; width: 80px;overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
																	<span v-for="user in time.userList"> <template
																			v-if="user.scopedSlots.title == 'custom'"> {{user.title}} </template></span>
																</span>
															</a-tooltip>
										...
										<a-form-item v-if="current == 1" label="员工添加上限" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }">
										...
										<a-form-item style="padding-bottom: 20px;"
											...
											<template v-if="info.type == 2">
												...
												<a-tooltip placement="top">
													<template slot="title">
														<div>
															<span v-for="user in info.spare_employee"> <template
																	v-if="user.scopedSlots.title == 'custom'"> {{user.title}} </template></span>
														</div>
													</template>
													<span v-if="info.spare_employee.length > 0"
												...
												<a-tooltip placement="top">
													...
													<span v-if="info.spare_employee.length > 0"
													      style="display: inline-block; vertical-align: middle;">
																<template v-for="(user, index) in info.spare_employee">
																	<a-tag color="orange"
																	       v-if="user.scopedSlots.title != 'custom' && getThreePartyIndex(info.spare_employee) >= index">{{user.title}}</a-tag>
																</template>
										...
										<!-- 渠道欢迎语 -->
										<a-form-item v-if="current == 2" label="扫码推送" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }">
										...
										<a-tabs tabPosition="left" style="margin-top: 20px;" default-active-key="1"
										        @change="changeWelcomeTabKey"
										        v-if="pushValue && current == 2">
											<a-tab-pane key="1" tab="通用欢迎语">
												<a-form-item v-if="pushValue && current == 2"
													...
													<div style="width: 900px; background-color: #FFFFFF;padding: 10px 0;margin-top: 10px">
														...
														<div class="pull-right fl">
															<cEditor v-if="editorFlag" :text="textContent"
																		...
																		<div class="msg_content_body">
																			...
																			<div v-if="item.content.img && pushValue"
																				...
																				<a-avatar
																						...
																						:size="36"
																						style="margin-right: 10px;float: left;object-fit: cover;"/>
																				<img :src="commonUrl+item.content.img"
																				     alt
																				     style="width: 150px;border-radius: 4px;"/>
																			</div>
																			<!--网址-->
		...
		<!-- 选择成员 -->
		<chooseDepartment ref="user" :show="showModalDepartment" :is_from="7" :departmentDisabled="departmentDisabled"
	...
	export default {
		...
		data () {
			...
			return {
				...
				info                 : {
					...
					scene          : 2,
					remark         : '',
					skip_verify    : 0,  // 是否开启添加好友验证
					verify_all_day : 1, // 添加好友验证类型
					state          : '',
					style          : 1,
					user           : [],  // 单人选中的成员
					specialTime    : 0, // 是否开启特殊时期
					// 周期数据
					specialWeekList: [
						{
							mon   : [],
					...
					party          : [], // 部门
					tag_ids        : [], // 标签数组
					is_welcome_week: 1,  // 周欢迎语
					is_welcome_date: 1,  // 是否开启特殊时间欢迎语
				},
				// 开启添加好友验证的时间段
				timeList             : [{
				...
				weekTabKey           : 0,  // 周期欢迎语tabKey
				weekKey              : 0,  // 特殊时期欢迎语tabKey
				// 弹窗周期成员数据
				welcome_week_list    : [
					{
						date: [],
				...
				dateTabKey           : 0, //  特殊时期欢迎语 tab key
				dateKey              : 0, //  强制特殊时期欢迎语模块重新渲染
				// 特殊时期欢迎语数据
				welcome_date_list    : [
					{
						date: [],
				...
				specialTimeTitle     : '添加企业成员', // 周期添加、编辑成员弹窗标题
				// 方便逻辑处理
				weekList             : [
					{
						key : 'mon',
				...
				weekDayKey           : [], // 周期成员弹窗选中的周期

				weekChecked               : new Array(7).fill(false), // 周期成员弹窗周期选中情况
				// 成员上限表格colums
				limitUserColumns          : [
					{
						title    : "名称",
				...
				specialTimeUserList       : [], // 周排班弹窗数据
				timeIndex                 : "", // 特殊时期选择成员的时间index
				currentType               : '', // 1周期、2特殊时期选择成员
				currentIndex              : '', // 周期、特殊时期选择成员的index
				// 特殊时期成员数据
				specialTimeList           : [
					{
						startTime : moment('00:00', 'HH:mm'),
				...
				showModalApplet           : false,//小程序弹窗的显示与隐藏
				miniproAddType            : 1, // 新建或导入
				miniproAddType1           : 1, // 小程序新建或导入
				material_id3              : '', // 小程序素材id
				showModalMinipro          : false, // 导入框
				appletUrl                 : '',//小程序的封面链接
				appletInputTitle          : '',//小程序的标题
				appid                     : '',//小程序的appid
				...
				departmentDisabled        : 0, // 选择成员组件是否禁选部门
				checkedList               : [],//选择的成员id数组
				welcomeDisabled           : false,//通用按钮的置灰
				chooseNum                 : 0,//已选择的成员 + 部门数量
				chooseUserNum             : 0, //  已选择的成员数量
				chooseDepartmentNum       : 0, // 已选择的部门数量
				selectGroupId             : '',  // 同步的内容引擎分组id
				from_channel              : 2,
				disabledSync              : 0,     // 同步是否禁选
				materialSync              : 0,     // 是否同步
				radar_open                : 1,     // 是否开启雷达链接
				radar_dynamic_notification: 1,     // 动态通知
				radar_tag_open            : 1,     // 雷达客户标签
				tagVisible                : false, // 标签弹窗
				stag_arr                  : [],    // 标签数组
				radar_tag_ids_name        : [],    // 标签名数组
				radar_tag_ids             : '',    // 标签字符串
				// 弹窗参数合集
				materialSync1             : 0,     // 素材弹窗是否同步内容引擎
				disabledSync1             : 0,     // 编辑时是同步控制禁选
				...
				tagName2                  : [],    // 标签组件选中标签名
				material_id0              : '',    // 图文、小程序新建封面的素材id
				setIsShow                 : false, // 弹窗是否展示图文具体信息
				msgUrl1                   : '',    // 弹窗图文新建封面的url
				inputTitle1               : '',    // 弹窗图文新建标题
				digest1                   : '',    // 图文新建标题
				timeOut                   : false, // 图文弹窗  链接获取信息定时器
				url1                      : '',    // 图文弹窗  链接值
				material_id11             : '',    // 图文导入弹窗的素材id
				selectGroupId1            : '',    // 弹窗 图文、小程序同步选择的分组id
				appletUrl1                : '',    // 弹窗 小程序封面
				appletInputTitle1         : '',    // 弹窗 小程序标题
				appid1                    : '',    // 弹窗 小程序appid
				pageUrl1                  : '',    // 弹窗 小程序页面路径
				material_id31             : '',    // 小程序导入弹窗的素材id
				comefrom                  : '',
				isShowTag                 : false, // 展示选择标签组件
				welcomeType               : '',    // 欢迎语类型 周期、特殊时期
				weekIndex                 : '',    // 周期、特殊时期的欢迎语index
				wTimeIndex                : ''     // 周期、特殊时期的欢迎语时间index
			}
		},
		...
		methods: {
			...
			changeCurrent (index) {
				...
				localStorage.setItem('welcomeType', '-1')
				localStorage.setItem('welcomeIndex', '-1')
				localStorage.setItem('welcomeIdx', '-1')
			},
			moment,
			//素材同步
			changeMaterialSync () {
			...
			changeWelcome (content, welcome_content) {
				...
				content.radar_tag_ids = welcome_content.radar_tag_ids
				content.radar_tag_ids_name = welcome_content.radar_tag_ids_name
				content.radar_tag_open = welcome_content.radar_tag_open
				content.radar_open = welcome_content.radar_status == 1 && welcome_content.radar_id > 0 ? 1 : 0
				content.radar_dynamic_notification = welcome_content.dynamic_notification
				content.stag_arr = welcome_content.radar_tag_ids ? welcome_content.radar_tag_ids.split(',') : []
			...
			async saveWay () {
				...
				sendData['sub_id'] = localStorage.getItem('sub_id') || ''
				sendData['isMasterAccount'] = localStorage.getItem('isMasterAccount')
				...
				if (res.error != 0) {
					...
					if (typeof this.urlId != "undefined") {
						//编辑
						this.$router.push("/channelCode/list?isRefresh=1" + "&groupId=" + this.info.way_group_id + "&id=" + this.urlId)
					} else {
						//新建
						this.$router.push("/channelCode/list?isRefresh=1" + "&groupId=" + this.info.way_group_id)
					}
				}
			},
			//更换公众号
