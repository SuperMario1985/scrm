					...
					<a-layout-content>
						<div class="content-msg">
							<p style="margin-bottom: 2px;">
								针对某时间段内，粉丝向公众号发送消息后，提前设置好一套标准的自动回复。比如，有时候粉丝夜间向公众号发送信息，而工作人员早已下班，此时即可设置一个夜间留言提示，告知“等上班后，再回复您”等类似留言回复，这样粉丝就不用焦急等待了。</p>
							<p style="margin-bottom: 2px;">1）粉丝向公众号发送消息后，立即收到消息回复，含图文、图片、文字（可添加超链接）、音频、视频等。</p>
							<p style="margin-bottom: 2px;">2）根据当天第一次向公众号发送消息的时间点开始计入，在48小时内，可定时向粉丝自动回复消息。</p>
							<p style="margin-bottom: 0px;">3）<span style="color: #F56C6C;">推送优先级：如果当前时间点，有多条推送任务，那么针对粉丝而言，仅执行一个任务进行推送，以最后一个创建的任务时间推送。</span>
							</p>
						</div>
						...
						<div class="content-bd">
							<a-spin tip="Loading..." size="large" :spinning="isLoading">
								<a-table :columns="reply.columns" :dataSource="reply.list" :pagination="false"
								         :rowClassName="rowClassName" v-has="'message-list'">
									...
									<span slot="action" slot-scope="text, record,index">
					                    <a-button style="margin-right: 5px; margin-bottom: 5px;"
					                              @click="statistical(record.id)"
					                              v-has="'message-statistic'">统计</a-button>
					                    <a-button style="margin-right: 5px; margin-bottom: 5px;"
					                              @click="pushLog(record.id)" v-has="'message-push'">推送日志
					                    </a-button>
										<a-button style="margin-right: 5px; margin-bottom: 5px;"
										          v-if="record.is_show==1" @click="edit(record.id)">编辑
					                    </a-button>
										<!--					                    <a-button style="margin-right: 5px; margin-bottom: 5px;"-->
										<!--					                              @click="copy(record.id)">复制</a-button>-->
					                    <a-button style="margin-right: 5px; margin-bottom: 5px;"
					                              @click="deleteData(record.id)" v-has="'message-delete'">删除</a-button>
									</span>
								</a-table>
								<!-- 分页 -->
								<div style="width: 100%;position: absolute;margin: 20px 0px;" v-show="reply.total > 0"
								     v-has="'message-list'">
...
<script>
	...
	export default {
		...
		methods   : {
			...
			// 编辑
			edit (id) {
				this.$router.push({path: "/push/add", query: {id: id}});
			},
			// 复制
			// copy (id) {
			// },
		...
		beforeRouteEnter (to, from, next) {
			...
			// 不！能！获取组件实例 `this`
			// 因为当守卫执行前，组件实例还没被创建
			if (from.path == '/push/add' && typeof to.query.isRefresh != 'undefined' && to.query.isRefresh == '1') {
				next(vm => {
					//编辑回来
					vm.getReply(vm.page,vm.pageSize)
				});
			}else if (from.path == '/push/add' && (typeof to.query.isRefresh == 'undefined' || '1' != to.query.isRefresh) || from.path != '/push/add') {
				next(vm => {
					//因为当钩子执行前，组件实例还没被创建
