                      ...
                      <template>
                        ...
                        <div class="content-msg" style="font-size: 13px">
                          ...
                          >
                            ...
                            <a
                              :href="commonUrl + '/upload/slider.png'"
                              target="_blank"
                ...
                <div class="content-bd">
                  <a-form>
                    ...
                    >
                      ...
                      <span
                        style="
                          ...
                          color: #f56c6c;
                        "
                        >{{ msg_title === null ? 0 : msg_title.length }} /
                        20</span
                      >
                      <span style="color: #f56c6c; margin-left: 20px"
                        >（一旦发布，不可修改）</span
                      ...
                      >
                        <template v-for="(item, index) in clockInList">
                          <a-tab-pane
                            ...
                            :key="item.key"
                            :closable="
                              !(status != 0 && item.id != null && item.id != '')
                            "
                            style="padding: 0 10px"
                          >
                            <div style="color: #f56c6c; margin-bottom: 5px">
                              每个打卡阶段任务只可奖励1次。一旦关闭，该打卡任务不在手机端展示，但对已获得奖品的用户依然展示。
                            </div>
                            <div>
                              <a-switch v-model="item.is_open" />
                            </div>
                            <div>
                              用户<span v-if="clockInType == 1">连续</span
                              ><span v-if="clockInType == 2">累计</span>打卡
                              <a-input-number
                                class="brush-input"
                                :min="0"
                                :max="99999999"
                              ...
                              <a-tooltip placement="top">
                                <template slot="title">
                                  <span
                                    >（修改时长后，不影响在修改前已获得奖品的用户。若设置连续3天打卡获得奖励，某用户已连续打卡2天，此时更改为“2天”，该用户依然无法获得奖励，但是若断掉后，再连续签到2天，即可获得。）</span
                                  >
                                </template>
                                <a-icon
                            ...
                            >
                              ...
                              元
                              <span style="color: #f56c6c"
                                >（一旦发布，不可修改。精确到小数点后两位，可输入0.30~5000.00）</span
                              >
                            </a-form-item>
                            <a-form-item
                              :label-col="{ span: 3 }"
                            ...
                            >
                              ...
                              <span
                                ...
                                "
                                >{{
                                  item.reward_name === null
                                    ? 0
                                    : item.reward_name.length
                                }}
                                / 10</span
                              >
                            </a-form-item>
                            ...
                            >
                              ...
                              >
                                ...
                                <a-radio :value="1" style="margin-top: 9px">
                                  ...
                                  <a-tooltip placement="bottom">
                                    <template slot="title">
                                      <span
                                        >当奖品是需要到店自提的奖品、或是以线下发放为主的虚拟奖品时，需要联系客服。</span
                                      >
                                    </template>
                                    <a-icon
                                      type="question-circle"
                              ...
                              <div
                                v-if="
                                  item.type == 2 ||
                                  (item.type == 1 && item.reward_type == 1)
                                "
                              >
                                <div>
                                  <a-button
                                  ...
                                  >
                                    <span v-if="item.userKeyNum > 0"
                                      >已选择{{ item.chooseUserNum }}名成员，{{
                                        item.chooseDepartmentNum
                                      }}个部门</span
                                    >
                    ...
                    <a-form-item style="margin-bottom: 0; padding-bottom: 10px">
                      <div
                        style="height: 20px; border-bottom: 1px solid #e0e0e0"
                      ></div>
  ...
  methods: {
    ...
    changeStartTime(date, dateString) {
      if (
        dateString &&
        this.startDate &&
        dateString > moment(this.endDate).format("YYYY-MM-DD HH:mm")
      ) {
    ...
    changeEndTime(date, dateString) {
      if (
        dateString &&
        this.startDate &&
        dateString < moment(this.startDate).format("YYYY-MM-DD HH:mm")
      ) {
        this.$message.destroy();
    ...
    addSetting() {
      ...
      let newTabIndex = Number(
        this.clockInList[this.clockInList.length - 1].key
      );
    ...
    showDepartmentList(index) {
      ...
      this.$refs.user.rightList = JSON.parse(
        JSON.stringify(this.clockInList[index].user_keys)
      );
    ...
    modalVisibleChange3(event, arr, userNum, departmentNum, list) {
      if (event == "ok") {
        this.clockInList[this.chooseIndex].userKeyNum =
          parseInt(userNum) + parseInt(departmentNum);
    ...
    async submit() {
      ...
      for (let i = 0; i < this.clockInList.length; i++) {
        if (
          this.clockInList[i].days == "" ||
          this.clockInList[i].days == null
        ) {
          ...
          if (
            typeof this.clockInList[i] != "undefined" &&
            typeof this.clockInList[i].days != "undefined" &&
            typeof this.clockInList[i - 1] != "undefined" &&
            typeof this.clockInList[i - 1].days != "undefined"
          ) {
            if (this.clockInList[i].days <= this.clockInList[i - 1].days) {
              this.$message.warning(
                "第" + (i + 1) + "阶段打卡天数必须大于前一阶段"
              );
              return false;
            }
          }
        ...
        if (this.clockInList[i].type == 1) {
          //实物
          if (
            this.clockInList[i].reward_name == "" ||
            this.clockInList[i].reward_name == null
          ) {
          ...
          if (this.clockInList[i].reward_type == 1) {
            //联系客服
            if (
              this.clockInList[i].userKeyNum == 0 ||
              this.clockInList[i].user_keys.length == 0
            ) {
          ...
          //红包
          if (
            this.clockInList[i].money_amount == "" ||
            this.clockInList[i].money_amount == null
          ) {
      ...
      const { data: res } = await this.axios.post(
        "work-group-clock-activity/add",
        {
          ...
          start_time:
            this.startDate != null
              ? this.startDate.format("YYYY-MM-DD HH:mm")
              : "",
          end_time:
            this.endDate != null ? this.endDate.format("YYYY-MM-DD HH:mm") : "",
          rule: this.rule,
    ...
    async getDetail() {
      this.isLoading = true;
      const { data: res } = await this.axios.post(
        "work-group-clock-activity/detail",
        {
          id: this.urlId,
        }
      );
      if (res.error != 0) {
        ...
        if (this.activeType == 2) {
          this.startDate =
            res.data.start_time != null && res.data.start_time != ""
              ? moment(res.data.start_time)
              : null;
          this.endDate =
            res.data.end_time != null && res.data.end_time != ""
              ? moment(res.data.end_time)
              : null;
          this.endDate2 =
            res.data.end_time != null && res.data.end_time != ""
              ? moment(res.data.end_time)
