      ...
      <a-layout style="position: relative; height: 100%; overflow: hidden">
        ...
        >
          ...
          <a-layout-content>
            ...
            >
              <a
                href="https://support.qq.com/products/104790/faqs/85022"
                target="_blank"
              >
            ...
            >
              ...
              <p style="margin: 10px 0 2px">流程如下：</p>
              ...
              <p style="margin-bottom: 2px">
                3、同时，使用客户群
                群发工具，由企业管理员或是员工群发，将打卡活动群发到客户群中，邀请客户参与。
              </p>
            ...
            <div class="content-hd">
              <a-col style="float: left">
                ...
                <a-range-picker
                  ...
                  @change="changeTime"
                />
                <a-button type="primary" style="margin: 0px 10px" @click="find"
                  >查询</a-button
                >
              ...
              <a-col style="float: right">
                <a-button
                  v-has="'groupClockIn-add'"
                  class="btn-primary"
                  @click="addWelcomeText"
                  type="primary"
                >
                  创建活动
                </a-button>
            ...
            <div class="content-bd">
              <a-spin tip="加载中..." size="large" :spinning="isLoading">
                <a-table
                  ...
                  v-has="'groupClockIn-list'"
                >
                  <span slot="url" slot-scope="text, record, index">
                    <div
                      style="width: 100px; margin: 0 auto 6px; float: left"
                      :ref="'qrcode' + index"
                      :id="'qrcode' + index"
                    ></div>
                  ...
                  <span slot="title2" slot-scope="text, record">
                    <div style="width: 140px; margin-right: 15px">
                      {{ record.title }}
                    </div>
                  </span>
                  <span slot="date_str" slot-scope="text, record">
                    <div v-if="record.type == 1" style="width: 150px">
                      永久有效
                    </div>
                  ...
                  <span slot="taskRule" slot-scope="text, record">
                    <div v-for="item in record.task">
                      ...
                      <span v-if="record.choose_type == 2">累计打卡</span
                      ><span style="color: red">{{ item.days }}</span
                      >天，赠送<span v-if="item.type == 1">{{
                        item.reward_name
                      }}</span
                      ><span v-if="item.type == 2"
                        >红包{{ item.money_amount }}元</span
                      >
                    </div>
                  </span>
                  ...
                  >
                    ...
                    <a-button
                      style="margin: 0 5px 5px 0"
                      v-has="'groupClockIn-detail'"
                      @click="openDetail(record.id, record.title)"
                      v-if="record.status != 0"
                      >参与明细
                    </a-button>
                    <a-button
                      :data-url="record.qr_code"
                      v-has="'groupClockIn-load'"
                    ...
                    <a-button
                      @click="deleteActivity(record.id, record.title)"
  ...
  methods: {
    ...
    async getActivityList(page = 1, pageSize = this.pageSize) {
      ...
      const { data: res } = await this.axios.post(
        "work-group-clock-activity/list",
        {
          corp_id: this.corpId,
          status: this.status,
          title: this.title,
          start_time:
            this.activityTime.length > 0
              ? moment(this.activityTime[0]).format("YYYY-MM-DD")
              : "",
          end_time:
            this.activityTime.length > 0
              ? moment(this.activityTime[1]).format("YYYY-MM-DD")
              : "",
          page: page,
          page_size: pageSize,
        }
    ...
    async changeStatus(id, status) {
      const { data: res } = await this.axios.post(
        "work-group-clock-activity/invalid",
        {
          id: id,
          type: status,
        }
