      ...
      <a-layout style="position: relative">
        ...
        >
          ...
          <a-layout-content>
            ...
            >
              <p style="margin-bottom: 2px">
                <span style="color: #ff562d"
                  >自动拉群，不仅可以完全实现不需要员工手工拉客户进群，而且完全打破企业微信官方只能选择5个群聊的局限，流程如下：</span
                >
                <a-popover placement="bottom">
                  <template slot="content">
                    <img
                      style="width: 1122px; height: 444px"
                      src="../../../assets/group/step.png"
                    />
                  </template>
              ...
              <p style="margin-bottom: 2px">
                1、在本系统添加多个群聊的二维码（群聊数量足够），添加完成后会生成一个渠道活码，一个渠道活码对应一个或多个使用成员（相当于客服）。
              </p>
              <p style="margin-bottom: 2px">
                2、当客户扫该渠道活码之后就会先添加使用成员，同时客户会收到管理员提前设置的入群引导语，引导语中包括入群二维码，然后扫码进群。
              </p>
              <p style="margin-bottom: 2px">
                3、当第一个群达到人数上限（最多200人）后，客户扫码添加使用成员后，推送第二群的入群二维码，客户扫码进入第二个群聊，依次往后进行。
              </p>
              <p style="margin-bottom: 0px; color: #ff562d">
                在整个流程中，即使在客户不入群或当客户退群时，该客户其实就已添加过客服，可以保持单独联系，完善客户画像，提供一对一的精准服务。
              </p>
            </div>
            <div class="content-hd">
              <a-col style="float: left">
                ...
                <a-button
                  ...
                  >查询</a-button
                >
                <a-button style="margin-left: 10px" @click="clearTitle"
                  >重置</a-button
                >
              </a-col>
                ...
                >
                  ...
                  <span slot="chat_list" slot-scope="text, record, index">
                    <div v-for="item in text">
                      ...
                      <a-tag v-if="item.chat_status == 1" color="green"
                        >拉人中</a-tag
                      >
                      <a-tag v-if="item.chat_status == 2" color="red"
                        >已满群</a-tag
                      >
                      <a-tag v-if="item.chat_status == 3" color="orange"
                        >已停用</a-tag
                      >
                    </div>
                  </span>
                  <span slot="action" slot-scope="text, record, index">
                    <div style="max-width: 140px">
                      <a-button
                        @click="detail(record.id, record.title)"
                        class="actionBtn"
                        v-has="'groupAutoCreate-info'"
                        >详情</a-button
                      >
                      <a-button
                        @click="edit(record.id)"
                        class="actionBtn"
                        v-has="'groupAutoCreate-edit'"
                        >编辑</a-button
                      >
                      <a-button
                        :data-url="commonUrl + record.local_path"
                        :data-name="record.title"
                        @click="downLoadWay"
                        class="actionBtn"
                        v-has="'groupAutoCreate-down'"
                        >下载</a-button
                      >
                      <a-button
                        @click="move(record.id, record.way_group_id, 0)"
                        class="actionBtn"
                      ...
                      <a-button
                        @click="
                          definite(record.id, record.title, record.chat_list)
                        "
                        class="actionBtn"
                        v-has="'groupAutoCreate-detail'"
                        >明细</a-button
                      >
                    </div>
                  </span>
              ...
              <a-spin tip="加载中..." size="large" :spinning="isLoading2">
                <div class="detailBox-left">
                  <div class="left-one">
                    ...
                    <div class="one-date">
                      创建时间：{{ detailData.create_time }}
                    </div>
                    <div class="one-name">{{ detailData.title }}</div>
                    <a-button
                      :data-url="commonUrl + detailData.local_path"
                  ...
                  <div class="left-two">
                    ...
                    <div class="tags-list">
                      <p
                        ...
                        v-if="
                          detailData.tag_name && detailData.tag_name.length == 0
                        "
                      >
                        暂无标签
                      </p>
                      <a-tag color="blue" v-for="item in detailData.tag_name">{{
                        item
                      }}</a-tag>
                    </div>
                  </div>
                  <div class="left-three">
                    <div
                      style="float: left; height: 100%; color: #1a1a1a"
                      ...
                      <template v-for="item in detailData.chat_list">
                        <a-tag style="padding: 8px">
                          <span
                            ...
                            "
                            v-if="
                              item.avatarData && item.avatarData.length == 0
                            "
                          >
                            <img
                              :src="img"
                          ...
                          <span
                            ...
                            "
                            v-else-if="
                              item.avatarData && item.avatarData.length == 1
                            "
                          >
                            <img
                              :src="item.avatarData && item.avatarData[0]"
                          ...
                          <span
                            ...
                            v-else-if="
                              item.avatarData &&
                              item.avatarData.length > 1 &&
                              item.avatarData.length <= 4
                            "
                          >
                          ...
                          <span
                            ...
                            "
                            v-else-if="
                              item.avatarData && item.avatarData.length > 4
                            "
                          >
                            <template v-for="imgItem in item.avatarData">
                              <img
                ...
                <div class="detailBox-right">
                  ...
                  <img
                    src="http://s.weituibao.com/static/1558341456351/qrcodedeyulan.png"
                    alt=""
                    class="preview-img"
                  />
          ...
          >
            ...
            <div
              class="group-tree"
              style="height: 270px; width: 100%; overflow-x: auto"
            >
              <a-tree
            ...
            <div style="padding: 20px">
              <a-row class="single-accout-card">
                <a-col :span="6" class="single-accout-cardItem">
                  ...
                  <p class="single-accout-cardItem-title">
                    累计添加客户数
                    <a-tooltip placement="right">
                      <template slot="title">
                        <span
                          >通过该活码，添加的成员客户数，包含删除员工，算累计值</span
                        >
                      </template>
                      <a-icon type="question-circle" style="margin-left: 5px" />
                    </a-tooltip>
                ...
                <a-col :span="6" class="single-accout-cardItem">
                  ...
                  <p class="single-accout-cardItem-title">
                    ...
                    <a-tooltip placement="right">
                      <template slot="title">
                        <span
                          >通过该活码，添加成员后进入该群的客户数，去掉其中入群再退群的人数</span
                        >
                      </template>
                      <a-icon type="question-circle" style="margin-left: 5px" />
                    </a-tooltip>
                ...
                <a-col :span="6" class="single-accout-cardItem">
                  ...
                  <p class="single-accout-cardItem-title">
                    ...
                    <a-tooltip placement="right">
                      <template slot="title">
                        <span
                          >通过该活码，添加成员后进入该群的客户数，其中入群再退群的人数</span
                        >
                      </template>
                      <a-icon type="question-circle" style="margin-left: 5px" />
                    </a-tooltip>
                ...
                <div style="background-color: #ffffff">
                  ...
                  <span class="select-option" style="margin-left: 15px">
                    ...
                    <a-button @click="showChatVisible" style="width: 180px">
                      <span style="text-align: center" v-if="chooseNum > 0"
                        >已选择{{ chooseNum }}个群</span
                      >
                  ...
                  <div style="margin-top: 10px">
                    <span class="select-option" style="margin-right: 15px">
                      ...
                      <a-button
                        ...
                        style="width: 180px"
                      >
                        <span v-if="chooseNum2 > 0"
                          >已选择{{ chooseUserNum2 }}名成员，{{
                            chooseDepartmentNum2
                          }}个部门</span
                        >
                    ...
                    <span class="select-option">
                      ...
                      >
                        <a-select-option :value="1"
                          >添加客服时间</a-select-option
                        >
                        <a-select-option :value="2"
                          >删除客服时间</a-select-option
                        >
                    ...
                    <a-button style="margin-right: 10px" @click="resetRecord"
                      >重置</a-button
                    >
                  </div>
                </div>
              ...
              >
                ...
                >
                  <div slot="customer_name" slot-scope="text, record, index">
                    ...
                    />
                    <div
                      style="float: left; margin-left: 10px; line-height: 42px"
                    >
                      <div style="height: 42px">
                        <a-popover placement="top">
                          <template slot="content">
                            ...
                            <span
                              :class="
                                record.corp_name != null
                                  ? 'corp-name'
                                  : 'wx-name'
                              "
                            >
                              <template v-if="record.corp_name != null"
                                >@{{ record.corp_name }}</template
                              >
...
export default {
  ...
  methods: {
    ...
    //部门选择弹窗组件
    showDepartmentList() {
      this.$refs.user.rightIdList = JSON.parse(
      ...
      if (res.error != 0) {
        ...
        if (this.batchType == "0") {
          if (this.checkArr.length > 0) {
            this.batchTypeValue = this.checkArr.every((val) =>
              arr.includes(val)
            );
          } else {
          ...
          if (this.keysArr.length > 0) {
            this.batchTypeValue =
              arr.sort().toString() == this.keysArr.sort().toString();
          } else {
            this.batchTypeDisabled = true;
          }
        }
    ...
    async getQRcode(scene_id) {
      const { data: res } = await this.axios.post("scene/down", {
        scene_id: scene_id,
        show: this.show,
      });
      // console.log(res)
    ...
    onSelectChange(selectedRowKeys) {
      ...
      if (this.batchType == "0") {
        this.batchTypeValue = this.checkArr.every((val) =>
          selectedRowKeys.includes(val)
        );
      } else if (this.batchType == "1") {
        this.batchTypeValue =
          selectedRowKeys.sort().toString() == this.keysArr.sort().toString();
      }
    },
    // 当前页多选框变化
    batchTypeChange() {
    ...
    changeBatchType() {
      ...
      if (!this.batchTypeDisabled) {
        if (this.batchTypeValue) {
          ...
          if (this.batchType == "0") {
            this.batchTypeValue = this.checkArr.every((val) =>
              this.selectedRowKeys.includes(val)
            );
          } else if (this.batchType == "1") {
            this.batchTypeValue =
              this.selectedRowKeys.sort().toString() ==
              this.keysArr.sort().toString();
          }
        }
      }
    },
    ...
    async getDefiniteList(page = 1, pageSize = this.pageSize2) {
      ...
      // }
      const { data: res } = await this.axios.post(
        "work-chat-way/detail-group-num",
        {
          corp_id: this.corpId,
          chat_ids: this.chatArr,
          ...
          start_time:
            this.joinTime == null
              ? ""
              : this.joinTime.length > 0
              ? moment(this.joinTime[0]).format("YYYY-MM-DD HH:mm")
              : "",
          end_time:
            this.joinTime == null
              ? ""
              : this.joinTime.length > 1
              ? moment(this.joinTime[1]).format("YYYY-MM-DD HH:mm")
              : "",
          type: this.isJoin,
  ...
  beforeRouteEnter(to, from, next) {
    ...
    if (
      from.path == "/group/add" &&
      typeof to.query.isRefresh != "undefined" &&
      to.query.isRefresh == "1"
    ) {
      next((vm) => {
        //编辑回来、返回回来
