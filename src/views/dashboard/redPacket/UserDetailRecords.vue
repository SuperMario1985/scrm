						...
						<div class="content-hd">
							<div style="background-color: #FFFFFF;padding: 20px 0 10px 0;margin-top: 12px;">
								...
								<span class="select-option">
									...
									<a-input
											style="margin-right: 5px;width: 210px;" @keyup.enter="searchStaff"
											v-model="phone"
								...
								<span class="select-option">
									...
									<a-input style="margin-right: 5px;width: 210px;" @keyup.enter="searchStaff"
									         placeholder="请输入备注/留言" v-model="remark"></a-input>
								</span>
								<span class="select-option">
									<label style="margin-right: 5px;">部门成员：</label>
									<a-button :disabled="true" @click="showDepartmentList"
									          style="width: 210px;margin-right: 5px;">
	...
	export default {
		...
		methods: {
			...
			reset () {
				...
				this.sendType = -1
				this.chatId = ''
				this.checkedList = this.$route.query.user_id
				this.chooseNum = 1
		...
		mounted () {
			this.$store.dispatch('getCorp', (info) => {
				...
				localStorage.setItem('backUrl', '/redPacket/list')
