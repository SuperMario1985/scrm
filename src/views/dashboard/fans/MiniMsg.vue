<template>
	<div ref="miniMsgBox" :style="{height: '100% !important'}">
		<a-tabs
				:style="{height: '100%'}"
				:activeKey="activeKey"
			...
			<a-tab-pane :key="changeKey">
				<template slot="tab">
					<span style="height: 35px;" class="tabMenu" @click="changeType('wx')" v-has="'fansMsg'">公众号</span>
					<span style="height: 35px;" class="tabMenu tabMenuBorder" v-has="'miniMsg'">小程序</span>
				</template>
			</a-tab-pane>
			...
			<a-tab-pane
				...
				<a-tabs
						v-if="msgTab.mini_list.length > 0"
						style="height: 100%;"
					...
					<a-tab-pane :key="msgTitleKey">
						<span slot="tab">用户消息</span>
					</a-tab-pane>

					<a-tab-pane
							v-for="(miniUser, mId) in msgTab.mini_list"
							:key="miniUser.openid"
							:style="subActiveKey == miniUser.openid ? 'height: 100%' : ''"
							:data-fans-id="miniUser.mini_id"
							:ref="miniUser.openid">
						<div slot="tab">
							<a-badge class="tab-box" :count="fansUnread[msgTab.id + '_' + miniUser.mini_id]">
								<a-avatar class="fans-avatar" :src="miniUser.head_img" shape="square"/>
								<div class="tab-info">
									<span class="fans-nick">{{miniUser.remark.length > 0 ? miniUser.remark + "（" +miniUser.openid + "）" : miniUser.openid}}</span>
									<span class="tab-notice"
									      v-html="tabText(msgTab.id, miniUser.mini_id, miniUser.openid)"></span>
								</div>
							</a-badge>
						</div>

						<a-row v-if="subActiveKey == miniUser.openid" class="chat-box" :gutter="16">
							<a-col :span="18" class="chat-info-box">
								<a-row class="chat-title">
									<a-col :span="22">
										<span class="fans-nick" v-show="!remarkVisible">
											{{miniUser.remark.length > 0 ? miniUser.remark + "（" +miniUser.openid + "）" : miniUser.openid}}
										</span>
										<span
												@click="modifyRemark(miniUser.remark.length)"
												v-show="!remarkVisible">
											<a-icon type="edit" alt=""/>
										</span>
										<a-input-search
												:defaultValue="miniUser.remark"
												placeholder="输入用户的备注"
												:data-wk="wId"
												:data-k="mId"
												:data-fid="miniUser.mini_id"
												@search="setRemark"
												@change="remarkChange"
												v-show="remarkVisible"
												:maxLength="10">
								...
								<a-row
									...
									<a-col :span="24">
										...
										<div class="nomore-box" v-if="noMore[msgTab.id + '_' + miniUser.mini_id]">
											<span>无更多消息</span>
										</div>
										<template v-for="msg in msgContent[msgTab.id + '_' + miniUser.mini_id]">
											<div :id="'contentList' + msg.id" class="chat-content-list"
											     :ref="msg.id">
												<a-divider
														dashed
														class="time-divider"
														v-if="showTimeDivider(msgTab.id, miniUser.mini_id, msg.id, msg.create_time)">
												...
												<div class="content">
													<a-avatar
															...
															:src="msg.from.data.head_img" class="float-left"/>
													<a-avatar
															shape="square"
								...
								<a-tag
										:visible="newMsg[msgTab.id + '_' + miniUser.mini_id]"
										class="new-msg-tag"
										color="blue"
										@click="newMsgChange">
									新消息
								...
								<a-row class="chat-send-box">
									<a-col :span="24">
										...
										<medium-editor
												ref="send_editor"
												class="send-textarea"
												:data-key="miniUser.mini_id"
												:data-active-key="msgTab.id"
												:text='valueInput[msgTab.id + "_" + miniUser.mini_id].input'
												:options="options"
										...
										<div class="send-btn-box float-right">
											...
											<a-button type="primary" size="large" :loading="sending" @click="reply"
											          :disabled="valueInput[msgTab.id + '_' + miniUser.mini_id].reply.length == 0 || valueInput[msgTab.id + '_' + miniUser.mini_id].reply == '\n'"
		...
		<a-modal
			...
			<a-carousel
				...
				<div v-for="(preview, key) in previewInfo[activeKey + '_' + miniId]">
					<img
	...
	export default {
		...
		data () {
			...
			return {
				wxId                   : '',    // 公众号唯一 ID
				miniId                 : 0, // 粉丝 ID
				msgFrom                : 2, // 消息发送方 2：小程序、3：客服 默认2
				msgType                : 1, // 消息类型 1：文本、2：图片、3：小程序
		...
		methods   : {
			setKeyGetContent (msgTab) {
				...
				this.wxId = msgTab.id
				if (msgTab.mini_list.length > 0) {
					this.subActiveKey = msgTab.mini_list[0].openid
					this.miniId = msgTab.mini_list[0].mini_id
			...
			initImgMsgContent (content, msgId) {
				...
				if (typeof content === "object" && typeof content[0] !== "undefined") {
					...
					content.map((item, index) => {
						let newImgDom = this.getImgDom(item, msgId + index)
						if (newImgDom != '') {
							hasNum++
							imgDom += newImgDom
						}
					...
					if (hasNum > 1) {
						imgDom = '<div class="media-content-box">' + imgDom + '</div>'
					}
				} else {
					imgDom = this.getImgDom(content, msgId)
				}

				if (imgDom == '') {
					imgDom = '<span class="wrong-notice">【图片无法显示】</span>'
				}
			...
			initPreviewInfo (msgId, content, type) {
				...
				if (typeof this.previewInfo[this.activeKey + '_' + this.miniId] === 'undefined') {
					this.previewInfo[this.activeKey + '_' + this.miniId] = [];
				}

				if (typeof this.previewRelation[this.activeKey + '_' + this.miniId] === 'undefined') {
					this.previewRelation[this.activeKey + '_' + this.miniId] = [];
				}

				if (typeof this.previewRelation[this.activeKey + '_' + this.miniId][msgId] === 'undefined') {
					this.previewInfo[this.activeKey + '_' + this.miniId].push(previewInfo);
					this.previewRelation[this.activeKey + '_' + this.miniId][msgId] = this.previewInfo[this.activeKey + '_' + this.miniId].length - 1;
				} else {
					this.previewInfo[this.activeKey + '_' + this.miniId][this.previewRelation[this.activeKey + '_' + this.miniId][msgId]] = previewInfo;
				}
			},
			// 绑定预览和音频播放暂停事件
			addComment (e) {
			...
			preview (e) {
				...
				this.$nextTick(() => {
					// 默认显示当前选中的资源
					this.$refs.previewCarousel.goTo(this.previewRelation[this.activeKey + '_' + this.miniId][e.target.dataset.key], false);
				});
			},
			// 关闭预览窗口
			handleCancel () {
			...
			// 选择器第二标签展示
			tabText (activeKey, miniId, tabKey) {
				if (this.subActiveKey != tabKey && typeof this.valueInput[activeKey + '_' + miniId] !== 'undefined' && this.valueInput[activeKey + '_' + miniId].reply.length != 0 && this.valueInput[activeKey + '_' + miniId].reply != '\n') {
					return '<span style="color: red">[草稿]</span>' + this.valueInput[activeKey + '_' + miniId].reply;
				}

				let fansMsgContent = '';
				let lastMsgIndex = -1;
				if (typeof this.msgContent[activeKey + '_' + miniId] === 'undefined' || this.msgContent[activeKey + '_' + miniId].length === 0) {
					fansMsgContent = this.msgTabList[this.fansLocation[activeKey + '_' + miniId].wx_index].mini_list[this.fansLocation[activeKey + '_' + miniId].fans_index].last_content;
					lastMsgIndex = fansMsgContent.length - 1;
				} else {
					fansMsgContent = this.msgContent[activeKey + '_' + miniId];
					lastMsgIndex = fansMsgContent.length - 1;
				}
			...
			// 获取粉丝消息
			getMsgContent () {
				if (typeof (this.valueInput[this.activeKey + '_' + this.miniId]) == 'undefined') {
					this.$set(this.valueInput, this.activeKey + '_' + this.miniId, {input: '', reply: '', item: ''});
				}
				if (typeof (this.noMore[this.activeKey + '_' + this.miniId]) == 'undefined') {
					this.$set(this.noMore, this.activeKey + '_' + this.miniId, false);
				}
				if (typeof (this.newMsg[this.activeKey + '_' + this.miniId]) == 'undefined') {
					this.$set(this.newMsg, this.activeKey + '_' + this.miniId, false);
				}

				if (typeof (this.msgContent[this.activeKey + '_' + this.miniId]) == 'undefined') {
					this.$set(this.msgContent, this.activeKey + '_' + this.miniId, []);

					this.getMsgContentList();
				} else {
					this.changeScrollTop(false, this.scrollData[this.activeKey + '_' + this.miniId].position);
					this.setNewMsg();
				}
			},
			// 获取最后的文本选择框焦点
			...
			insertHTMLCommand (doc, html, domType = 0) {
				if (domType == 0) {
					if (typeof self.lastNode === 'undefined' || !self.lastNode.classList.contains("send-textarea") || self.lastNode.dataset.key != this.miniId) {
						return false;
					}
				} else if (domType == 1) {
			...
			editableKeydownEnter (event) {
				if (event.shiftKey && event.keyCode === 13) {
					...
					// 发送处理
					if (!this.sending && this.valueInput[this.activeKey + '_' + this.miniId].reply.length != 0 && this.valueInput[this.activeKey + '_' + this.miniId].reply != '\n') {
						this.msgType = 1
						this.reply();
					}
				}
			...
			// 初始化时间线
			initTimeDivider (activeKey, miniId) {
				this.timeDivider[activeKey + '_' + miniId] = {
					time: [],
					show: []
				};
			},
			...
			// 判断时间线是否显示
			showTimeDivider (activeKey, miniId, msgId, time) {
				time = this.formatMsgTime(time);
				if (this.timeDivider[activeKey + '_' + miniId].time.indexOf(time) > -1 && this.timeDivider[activeKey + '_' + miniId].show[time] !== msgId) {
					return false;
				} else {
					if (this.timeDivider[activeKey + '_' + miniId].time.indexOf(time) === -1) {
						this.timeDivider[activeKey + '_' + miniId].time.push(time);
						this.timeDivider[activeKey + '_' + miniId].show[time] = msgId;
					}
			...
			handlePullOnLoad () {
				const scrollYReach = this.$refs.chat_content[0].$el._ps_.reach.y;
				if (scrollYReach === 'start' && !this.loading && !this.noMore[this.activeKey + '_' + this.miniId]) {
					this.loading = true;
					this.getMsgContentList(this.scrollData[this.activeKey + '_' + this.miniId].first_id);
				}
			},
			// 记录最后一次滚动条的位置
			changeScrollPosition (e) {
				this.scrollData[this.activeKey + '_' + this.miniId].position = e.srcElement.scrollTop;
			},
			// 点击新消息
			newMsgChange () {
			...
			changeScrollTop (change = false, scrollPosition = null) {
				this.$nextTick(() => {
					if (scrollPosition === null) {
						let msgId = this.scrollData[this.activeKey + '_' + this.miniId].old_first_id != 0 ? this.scrollData[this.activeKey + '_' + this.miniId].old_first_id : this.scrollData[this.activeKey + '_' + this.miniId].last_id;
						if (change) {
							msgId = this.scrollData[this.activeKey + '_' + this.miniId].last_id;
						}

						if (typeof this.$refs[msgId] !== 'undefined') {
							scrollPosition = this.$refs[msgId][0].offsetTop;

							if (msgId !== this.scrollData[this.activeKey + '_' + this.miniId].last_id) {
					...
					if (typeof this.$refs.chat_content !== 'undefined' && typeof this.$refs.chat_content[0] !== 'undefined') {
						...
						this.scrollData[this.activeKey + '_' + this.miniId].position = scrollPosition;
					}
				});
			},
			// 是否显示新消息tag
			setNewMsg () {
				this.$nextTick(() => {
					const msgId = this.scrollData[this.activeKey + '_' + this.miniId].last_id;
					...
					if (scrollPosition + msgBoxHeight > contentBoxHeight && this.scrollData[this.activeKey + '_' + this.miniId].position != scrollPosition) {
						let newBoxHeight = 0;
						let hasUnRead = false;
						if (typeof this.fansUnread[this.activeKey + '_' + this.miniId] != 'undefined' && this.fansUnread[this.activeKey + '_' + this.miniId] != 0) {
							hasUnRead = true;
							for (let i = 1; i <= this.fansUnread[this.activeKey + '_' + this.miniId]; i++) {
								let msgCount = this.msgContent[this.activeKey + '_' + this.miniId].length;
								let unReadMsgId = this.msgContent[this.activeKey + '_' + this.miniId][msgCount - i].id;
						...
						if (this.$refs.chat_content[0].$el.scrollTop + contentBoxHeight == this.$refs.chat_content[0].$el.scrollHeight || this.$refs.chat_content[0].$el.scrollTop + contentBoxHeight + newBoxHeight == this.$refs.chat_content[0].$el.scrollHeight) {
							...
							if (hasUnRead) {
								this.$set(this.newMsg, this.activeKey + '_' + this.miniId, true);
							}
						}
					} else {
						this.$set(this.newMsg, this.activeKey + '_' + this.miniId, false);
			...
			bindFansRelation () {
				...
				_this.msgTabList.map((item, wxIndex) => {
					...
					item.mini_list.map((fans, fansIndex) => {
						_this.fansLocation[item.id + '_' + fans.mini_id] = {
							wx_index  : wxIndex,
			...
			tabClick (key) {
				...
				if (typeof key !== 'undefined' && ignoreKey.indexOf(key) == -1 && key != this.activeKey) {
					...
					if (this.msgTabList[this.wxLocation[key]].mini_list.length > 0) {
						const subKey = this.msgTabList[this.wxLocation[key]].mini_list[0].openid;
						const subMiniId = this.msgTabList[this.wxLocation[key]].mini_list[0].mini_id;
						this.subTabClick(subKey, subMiniId)

						if (typeof this.scrollData[subMiniId] !== 'undefined') {
							this.changeScrollTop(false, this.scrollData[subMiniId].position);
						}
					} else {
						this.subActiveKey = 'empty';
					}
			...
			// subtab的点击
			subTabClick (key, miniId = null) {
				if (key != this.msgTitleKey && key != 'empty' && key != this.subActiveKey) {
					this.subActiveKey = key;
					this.miniId = miniId === null ? this.$refs[key][0].$attrs['data-fans-id'] : miniId;
					this.profileShow = false;
					this.getMsgContent();
				}
			},
			...
			quickInput (qid) {
				...
				this.valueInput[this.activeKey + '_' + this.miniId].input = this.$refs[quickRefKey][0].innerHTML;
				this.valueInput[this.activeKey + '_' + this.miniId].reply = this.$refs[quickRefKey][0].innerText;
			},
			// 通过快捷回复发送消息
			quickSend (qid) {
				this.quickInput(qid)
			...
			modalVisibleChange (event, e, id, item) {
				if (event == "ok") {
					...
					this.valueInput[this.activeKey + '_' + this.miniId].reply = id
					this.valueInput[this.activeKey + '_' + this.miniId].item = item
			...
			replySuccess (msgId, replyData) {
				...
				switch (replyData.msg_type) {
					case 1:
						...
						break;
					case 2:
						const imgList = this.valueInput[this.activeKey + '_' + this.miniId].item
						content = {
						...
						break;
					case 3:
						const local_url = this.valueInput[this.activeKey + '_' + this.miniId].local_url;
						const media_duration = this.valueInput[this.activeKey + '_' + this.miniId].media_duration;
						content = {
							local_url     : local_url,
						...
						break;
					case 4:
						const video_local_url = this.valueInput[this.activeKey + '_' + this.miniId].local_url;
						const video_media_duration = this.valueInput[this.activeKey + '_' + this.miniId].media_duration;
						content = {
						...
						break;
					case 5:
						const artList = this.valueInput[this.activeKey + '_' + this.miniId].item.artList[0]
						content = {
							article: [
								{
									thumb_info        : {
						...
						break;
					case 10:
						content = this.valueInput[this.activeKey + '_' + this.miniId].item.file_name;
				...
				let msgData = {
					...
					from       : {
						...
						data: {
							alias    : '',
							user_name: this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].id,
							nick_name: this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].name,
							head_img : this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].avatar,
						}
					},
					to         : {
						type: 1,
						data: {
							mini_id  : this.miniId,
							openid   : this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].mini_list[this.fansLocation[this.activeKey + '_' + this.miniId].fans_index].openid,
							nickname : this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].mini_list[this.fansLocation[this.activeKey + '_' + this.miniId].fans_index].nickname,
							remark   : this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].mini_list[this.fansLocation[this.activeKey + '_' + this.miniId].fans_index].remark,
							headerimg: this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].mini_list[this.fansLocation[this.activeKey + '_' + this.miniId].fans_index].headerimg,
						}
					},
					content    : content,
				...
				this.scrollData[this.activeKey + '_' + this.miniId].last_id = msgId;
				this.msgContent[this.activeKey + '_' + this.miniId].push(msgData);

				if (this.fansLocation[this.activeKey + '_' + this.miniId].fans_index != 0) {
					let fansInfo = this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].mini_list[this.fansLocation[this.activeKey + '_' + this.miniId].fans_index];
					this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].mini_list.splice(this.fansLocation[this.activeKey + '_' + this.miniId].fans_index, 1);
					this.msgTabList[this.fansLocation[this.activeKey + '_' + this.miniId].wx_index].mini_list.unshift(fansInfo);

					this.bindFansRelation();
				}

				this.initTimeDivider(this.activeKey, this.miniId);

				this.changeScrollTop(true);
			},
			// websocket获取到新消息的处理
			websocketOnMessage (data) {
				if (typeof data.type != 'undefined' && data.type == 'chat' && (!data.hasOwnProperty('msg_type') || data.msg_type == 'mini')) {
					...
					if (data.msg_list.type == 2) {
						data.msg_list.content = Object.values(data.msg_list.content)
					}
					...
					if (typeof this.fansLocation[data.wx_id + '_' + data.from] == 'undefined') {
						this.msgTabList[this.wxLocation[data.wx_id]].mini_list.unshift(data.mini_info);

						this.bindFansRelation();
					} else if (this.fansLocation[data.wx_id + '_' + data.from].fans_index != 0) {
						let fansInfo = this.msgTabList[this.fansLocation[data.wx_id + '_' + data.from].wx_index].mini_list[this.fansLocation[data.wx_id + '_' + data.from].fans_index];
						this.msgTabList[this.fansLocation[data.wx_id + '_' + data.from].wx_index].mini_list.splice(this.fansLocation[data.wx_id + '_' + data.from].fans_index, 1);
						this.msgTabList[this.fansLocation[data.wx_id + '_' + data.from].wx_index].mini_list.unshift(fansInfo);
					...
					// 非active的对话，添加未读数的徽标
					if (this.subActiveKey != data.mini_info.openid) {
						let unreadNum = 0;
			...
			async getMsgList () {
				...
				const {data: res} = await this.axios.post("mini-user/msg-list", params);
				if (res.error == 0) {
					this.changeLoading();
			...
			// 获取粉丝消息列表
			async getMsgContentList (lastId = 0, msgSize = 15, needChangeScroller = true, wxId = null, miniId = null) {
				if (wxId === null) {
					wxId = this.wxId;
				}
				...
				const {data: res} = await this.axios.post('mini-msg/get-msg-list', getData);
				if (res.error == 0) {
					if (res.data.msg_list.length > 0) {
						const msgMiniId = res.data.mini_id;
						const activeKey = res.data.wx_id
						const currentContent = this.msgContent[activeKey + '_' + msgMiniId];
						const oldFirstId = currentContent.length > 0 ? currentContent[0].id : 0;

						const _this = this;
						res.data.msg_list.map((msg) => {
							_this.msgContent[activeKey + '_' + msgMiniId].unshift(msg);
						});

						this.initTimeDivider(activeKey, msgMiniId);

						const firstId = this.msgContent[activeKey + '_' + msgMiniId][0].id;
						const lastIndex = this.msgContent[activeKey + '_' + msgMiniId].length - 1;
						const lastId = this.msgContent[activeKey + '_' + msgMiniId][lastIndex].id;

						if (res.data.unread != 0) {
							if (this.subActiveKey != this.msgTabList[this.fansLocation[activeKey + '_' + msgMiniId].wx_index].mini_list[this.fansLocation[activeKey + '_' + msgMiniId].fans_index].openid) {
								this.$set(this.fansUnread, activeKey + '_' + msgMiniId, res.data.unread);
							} else {
								this.msgRead(msgMiniId);
							}
						}
						...
						this.$set(this.scrollData, activeKey + '_' + msgMiniId, scrollInfo);
					} else {
						needChangeScroller = false
					}

					if (res.data.msg_list.length < msgSize) {
						this.$set(this.noMore, res.data.wx_id + '_' + res.data.mini_id, true);
					}
				...
				switch (type) {
					...
					case 2:
						...
						this.$confirm({
							...
							onOk () {
								_this.msgType = 2;
								_this.valueInput[_this.activeKey + '_' + _this.miniId].reply = item.id
								_this.valueInput[_this.activeKey + '_' + _this.miniId].item = item
								_this.uploadMedia({
									wx_id        : _this.activeKey,
			...
			async sendMsg (replyData) {
				const {data: res} = await this.axios.post('mini-msg/send', replyData);
			...
			async reply () {
				...
				if (typeof this.valueInput[this.activeKey + '_' + this.miniId].reply === 'number') {
					textValue = this.valueInput[this.activeKey + '_' + this.miniId].reply
				} else {
					textValue = this.valueInput[this.activeKey + '_' + this.miniId].reply.replace(/(^\s*)|(\s*$)/g, "")
				}
				if (textValue.length == 0 || !/[^\n]/g.test(textValue)) {
					...
					switch (this.msgType) {
						case 1:
							replyData.msg_content = {
								text: this.valueInput[this.activeKey + '_' + this.miniId].reply
							};
							break;
						case 2:
							replyData.msg_content = {
								media_id: this.valueInput[this.activeKey + '_' + this.miniId].reply
							};
							break;
					}
					const {data: res} = await this.axios.post('mini-msg/send', replyData);
			...
			async msgRead (miniId = null) {
				if (miniId === null) {
					miniId = this.miniId;
				}
				const {data: res} = await this.axios.post('mini-msg/msg-read', {
					mini_id: miniId,
					wx_id:localStorage.getItem('wxNum')
				});
				if (res.error == 0 && res.data.status) {
					this.$set(this.fansUnread, this.activeKey + '_' + this.miniId, 0);
				}
			},
			// 创建或者修改快捷回复
			...
			async setRemark (remark, e) {
				if (this.msgTabList[e.target.dataset.wk].mini_list[e.target.dataset.k].remark != remark) {
					let postData = {
						mini_id: e.target.dataset.fid,
						remark : remark,
						wx_id:localStorage.getItem('wxNum')
					};
					const {data: res} = await this.axios.post('mini-user/set-remark', postData);

					if (res.error == 0) {
						this.msgTabList[e.target.dataset.wk].mini_list[e.target.dataset.k].remark = remark
						this.remarkVisible = !this.remarkVisible;
					} else {
						this.$message.error(res.error_msg);
					}
