						...
						<a-layout-content>
							<div class="content">
								...
								<div class="content-bd">
									<a-form>
										<!-- 企业微信成员 -->
										<a-form-item label="企业微信成员" :label-col="{ span: 3 }"
										             :wrapper-col="{ span: 21 }" style="padding-top: 15px;">
											<a-button
											          @click="showDepartmentList">
												选择
											</a-button>
											<span style="margin-left: 10px;" v-show="chooseNum > 0">已选择{{chooseUserNum}}名成员，{{chooseDepartmentNum}}个部门</span>
										...
										<!-- 内容 -->
										<a-form-item label="欢迎语内容" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
										...
										<a-form-item>
											<div style="margin-top: 2px;margin-left: 12.5%;">
												<a-button type="primary" @click="createWelcome"
													...
													<template v-if="urlId == ''">创建</template>
													<template v-if="urlId != ''">修改</template>
													欢迎语
												</a-button>
											</div>
							...
							<WeChatModal :show="showModal2" :callback="modalVisibleChange"
										 hasName="welcome-add"></WeChatModal>
							<!-- 选择素材弹窗 -->
							<chooseMsg :show="showModal3" :type="typeValue2" :showRadar="true" :comefrom="comefrom" :news_type="news_type"
									   :callback="modalVisibleChange2" :sketchType="'1'"
		                    ...
		                    <span>{{ inputTitle1.length }}</span>/32
									...
									<a-form-item :label-col="{ span: 3 }"
										...
										给点击雷达的客户打上选中的标签
										<div>
											<a-button type="primary" @click="chooseTag">添加标签
											</a-button>
										</div>
							...
							<a-modal :visible="showModalApplet" title="小程序" @ok="handleApplet"
								...
								<template v-if="miniproAddType == 1">
									...
									<a-form-item :label-col="{ span: 3 }"
										...
										<span
											style="display: inline-block;margin-top: 64px;">（上传图片不超过1M，尺寸必须为1080*864像素，支持jpg/jpeg/png格式）</span>
									</a-form-item>
									<!-- 选中后 -->
									<a-form-item :label-col="{ span: 3 }"
												 :wrapper-col="{ span: 21 }"
	                    ...
	                    <span>{{ appletInputTitle1.length }}</span>/12
									...
									<a-form-item :label-col="{ span: 3 }"
										...
										<a-input style="margin-bottom: 10px;"
												 v-model="pageUrl1" placeholder="程序路径必须以“.html”结尾，例如：pages/index.html">
										</a-input>
									</a-form-item>
									<a-form-item label="素材同步" :label-col="{ span: 3 }"
												 :wrapper-col="{ span: 21 }">
							...
							<chooseDepartment ref="user" :show="showModalDepartment"
							                  :chooseNum="chooseNum" :welcome_id='Number(urlId)' :is_from="7"
							                  :callback="modalVisibleChange3" :disabledVal="1"  noticeTitle="只显示具有外部联系人权限的成员"></chooseDepartment>
...
export default {
	components: {
		WeChatModal, chooseMsg, chooseDepartment, MyIcon, chooseMinipro, cropperModal,
		editor, corpChooseTagModal
	},
	data () {
		return {
			editorFlag         : false,
			...
			corpInfo                 : [], //企业列表
			suite_id                 : 1,//应用ID
			corpId                   : localStorage.getItem('corpId') ? localStorage.getItem('corpId') : '',//企业微信选中的id
			corpName                 : localStorage.getItem('corpName') ? localStorage.getItem('corpName') : '',//企业微信选中的名字
	...
	methods: {
		goBack () {
			...
			this.$router.push("/welcome/list?isRefresh=1")
		},
		//素材同步
		changeMaterialSync () {
		...
		chooseUrl (index) {
			...
			this.radarOpen1 = this.radar_open
			this.radarDynamicNotification1 = this.dynamic_notification
			this.radarTagOpen1 = this.radar_tag_open
			this.tag_arr1 = JSON.parse(JSON.stringify(this.tag_arr))
			this.radarTagIds1 = JSON.parse(JSON.stringify(this.radar_tag_ids))
			this.tagName1 = JSON.parse(JSON.stringify(this.radar_tag_ids_name))
		...
		handleUrl () {
			...
			this.selectGroupId1 = this.selectGroupId
			this.setIsShow = false
			this.radar_open = this.radarOpen1
			this.dynamic_notification = this.radarDynamicNotification1
			this.radar_tag_open = this.radarTagOpen1
			this.tag_arr = JSON.parse(JSON.stringify(this.tag_arr1))
			this.radar_tag_ids = JSON.parse(JSON.stringify(this.radarTagIds1))
			this.radar_tag_ids_name = JSON.parse(JSON.stringify(this.tagName1))
			this.showModalUrl = false
		...
		handleCancelUrl () {
			if (!this.modalUrlOk) {
				...
				this.index = 0
				this.radarOpen1 = 1
				this.radarDynamicNotification1 = 1
				this.radarTagOpen1 = 1
				this.tag_arr1 = []
				this.radarTagIds1 = ''
				this.tagName1 = []
		...
		async uploadMedia (id) {
			const {data: res} = await this.axios.post("work-material/upload-media", {
				...
				attachment_id: id,
				type         : this.index == 1 ? 1 : (this.index == 2 && this.sketchAddType == 0 ? 4 : '')
			});
			if (res.error != 0) {
				this.$message.error(res.error_msg)
			} else {
		...
		handleApplet () {
			if (this.miniproAddType == 1) {
				if (this.appletUrl1 == '') {
					...
					return false;
				} else if (!this.pageUrl1.endsWith('.html')) {
					this.$message.warning('小程序路径须以“.html”结尾！')
					return false
				}
			} else {
				if (this.material_id31 == '') {
		...
		async createWelcome () {
			...
			const {data: res} = await this.axios.post("work-welcome/add", {
				uid                       : localStorage.getItem('uid'),
				corp_id                   : this.corpId,
				suite_id                  : this.suite_id,
				id                        : this.urlId,
				...
				radar_open                : this.radar_open,
				radar_dynamic_notification: this.dynamic_notification,
				radar_tag_open            : this.radar_tag_open,
				radar_tag_ids             : this.radar_tag_ids
			});
			if (res.error != 0) {
				this.$message.error(res.error_msg);
			} else {
		...
		async getDetail () {
			...
			if (res.error != 0) {
				...
				this.checkedList = res.data.users
				...
				this.radar_tag_ids = res.data.radar_tag_ids
				this.radar_tag_ids_name = res.data.radar_tag_ids_name
				this.radar_tag_open = res.data.radar_tag_open
				this.radar_open = res.data.radar_status == 1 && res.data.radar_id > 0 ? 1 : 0
				this.dynamic_notification = res.data.dynamic_notification
				this.tag_arr = res.data.radar_tag_ids ? res.data.radar_tag_ids.split(',') : []
				this.isLoading = false;
			}
		},
		modalVisibleChange (type, corpId, corpName) {
			if (type == 'ok') {
				if (this.corpId != corpId) {
					...
					for (let i = 0; i < this.corpArr.length; i++) {
						if (this.corpArr[i].corpid == this.corpId) {
							if (this.corpArr[i].welcome == 1) {
		...
		modalVisibleChange3 (event, arr, userNum, departmentNum, list) {
			if (event == "ok") {
				...
				if (this.urlId) {
					this.selectedUser = this.checkedList
				}
			}
	...
	created () {
		...
		if (this.$route.query.hasAll) {
			this.hasAll = this.$route.query.hasAll
		}
		...
		if (this.urlId != '') {
			...
			localStorage.setItem('backUrl', '/welcome/list')
			this.getDetail()
		} else {
			this.editorFlag = true
		}
