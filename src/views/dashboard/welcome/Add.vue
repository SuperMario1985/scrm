									...
									<a-form>
										...
										<a-form-item label="企业微信成员" :label-col="{ span: 3 }"
											...
											<a-radio-group name="radioGroup" v-model="validity"
											               :disabled="urlId=='' && hasAll == 0?false:true">
												<a-radio :value="1" :disabled="welcomeDisabled">通用</a-radio>
												<a-radio :value="2">仅企业成员适用
													<a-button style="margin-left: 10px;" :disabled="validity == 1"
	                    ...
	                    <span>{{appletInputTitle1.length}}</span>/12
									...
									<a-form-item :label-col="{ span: 3 }"
										...
										<a-input style="margin-bottom: 10px;"
										         v-model="pageUrl1" placeholder="请填写小程序AppID，必须是关联到企业的小程序应用">
										</a-input>
									</a-form-item>
									<a-form-item label="素材同步" :label-col="{ span: 3 }"
									             :wrapper-col="{ span: 21 }">
							...
							<chooseDepartment ref="user" :show="showModalDepartment"
							                  ...
							                  :callback="modalVisibleChange3" :disabledVal="1"  noticeTitle="只显示具有外部联系人权限的成员" :departmentDisabled="1"></chooseDepartment>
...
<script>
	...
	export default {
		...
		methods   : {
			...
			//创建、修改欢迎语
			async createWelcome () {
				if (this.validity == 2 && this.checkedList.length == 0) {
