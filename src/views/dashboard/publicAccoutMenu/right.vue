        ...
        <div class=" mt30 mb10 clearfix f14" v-if="currentMenu.sub_button && currentMenu.sub_button.length == 0 && currentMenu.leveltype == 'first' || currentMenu.leveltype == 'second' || !currentMenu.sub_button">
          ...
          <div v-if="contentType==3" class="content_type_one">
            <a-form-item :label-col="{ span: 4 }"
              ...
              <a-input style="width: 400px;"  v-model="minProgramAppid" @blur="inputChangeAppId"  placeholder="请填写小程序AppID，必须是关联到当前公众号的小程序应用"></a-input>
            ...
            <a-form-item :label-col="{ span: 4 }"
              ...
              <a-input style="width: 400px;"  v-model="pagePath" @blur="inputChangePath" placeholder="请输入小程序的页面路径，必须是关联到当前公众号的小程序应用"></a-input>
            </a-form-item>
            <a-form-item :label-col="{ span: 4 }"
                         :wrapper-col="{ span: 20 }">
              <template slot="label" ><span
          ...
          <div v-if="contentType==2" class="content_type_one">
            <a-form-item :label-col="{ span: 4 }"
              ...
              <a-input style="width: 400px;"  v-model="netUrl" @blur="inputChange" placeholder="请输入网页链接，且必须以http://或https://开头"></a-input>
            </a-form-item>
...
name: "right",
    ...
    currentMenu: {
      handler (newVal, oldVal) {
        if (JSON.stringify(newVal) != JSON.stringify(oldVal) ) {
          this.netUrl=''
          this.backUpUrl=''
          this.list = newVal.menu_content?_.cloneDeep(newVal.menu_content):[_.cloneDeep(templateObj)]
          //当前菜单切换时，需要把菜单下的tab重置
          this.current = 0
          this.contentType = newVal.content_type
         if(!newVal.appid){
  ...
  mounted() {
    this.originType=this.currentMenu.content_type
...
//菜单内容类型变更
    changeContentType(){
      ...
      switch (this.contentType){
          ...
          this.$store.commit('updateCurrentMenu',this.currentMenu)
    },
    inputChangeAppId(){
    ...
    inputChangeBackUpUrl(){
      this.currentMenu.url = this.backUpUrl
      this.$store.commit('updateCurrentMenu',this.currentMenu)
    },
    //验证网址
    inputChange(e){

      if(this.netUrl.trim().length==0){
    ...
    changeTitle(val) {
      ...
      this.$store.commit('updateCurrentMenu', currentM)
    },
    delMenu() {
      ...
      if (c.leveltype == 'first') {
        ...
        this.$confirm({title:`删除后“${c.name}”菜单下设置的内容将被删除`,onOk(){
            ...
            if(m.length>0){
              that.$store.commit('updateCurrentMenu',m[m.length-1])
              that.$store.commit('updateMenuList', m)
              that.$store.commit('updateActiveMenu', m.length-1)
            }else {
              that.$store.commit('clearCurrentMenu')
              that.$store.commit('updateMenuList', m)
              that.$store.commit('updateActiveMenu', null)
            }
        ...
        this.$confirm({
          ...
          onOk() {
            child.splice(idx, 1)
            if(child.length>0){
              that.$store.commit('updateCurrentMenu',child[child.length-1])
              that.$store.commit('updateMenuList', m)
              ...
              that.$store.commit('updateCurrentMenu',currentMenu)
              that.$store.commit('updateMenuList', m)
              that.$store.commit('updateActiveMenu', m.length-1)
            }
          },
