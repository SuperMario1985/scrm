  ...
  <div class="flex">
    ...
    <div class="menufirst flex flex-1">
      <div class="flex-1 firstitem cp"  v-for="(item, index) in menulist" :index="index" :key="index" :item="item" @click="changeActive(index)" :class="{active:currentMenu && item.timeid == currentMenu.timeid}">
        <div class="ellipsis w80">{{item.name}}</div>
...
name: "fixMenu",
  ...
  methods:{
    changeActive(idx){
      this.$store.commit('changeSpecialCom')
    ...
    addFirst(){
      ...
      this.$store.commit('updateSpecialactiveMenu',l)
      this.$store.commit('updateSpecialCurrentMenu',params)
      // this.save()
    },
    addSecond(){
      ...
      if(c.leveltype == 'first' && c.timeid ){
        ...
        this.$confirm({title:'添加子菜单后，一级菜单的内容将被清除。确定添加子菜单？',onOk(){
            ...
            delete(c['events'])
            delete(c['link'])
            that.$store.commit('updateSpecialCurrentMenu',c)
            that.addTwo()
          },})
    ...
    changeChild(idx){
      ...
      this.$store.commit('updateSpecialCurrentMenu',parentmenu.sub_button[idx])
    },
  },
}
</script>
