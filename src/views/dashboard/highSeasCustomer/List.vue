<template>
	<div style="width: 100%;max-height:100%;position: absolute;overflow-y: auto;padding-bottom: 30px;" class="scroll">
								...
								>
									<template slot="customerInfo" slot-scope="text, record, index">
										<div style="overflow: hidden;">
											...
											<div style="float: left;margin-left:10px;line-height: 42px;max-width: calc(100% - 52px);">
												<div style="height:42px;">
													<a-popover placement="top">
														...
														<span style="display: inline-block; max-width: 100%;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;">
														{{ text.name }}
														<span :class="text.corp_name != null ? 'corp-name' : 'wx-name'">
															<template
																v-if="text.corp_name != null">@{{ text.corp_name }}</template>
						...
						<a-spin tip="加载中..." size="large" :spinning="isLoading3">
							<div class="spin-content">
								<a-table
									...
									:pagination="false"
									:rowClassName="rowClassName" v-has="'highSeasCustomerList-list'" class="repeat-table"
								>
									<template slot="customerInfo" slot-scope="text, record, index">
										<div style="overflow: hidden;">
						...
						<div class="spin-content">
							...
							>
								...
								<template slot="tagData" slot-scope="text, record, index">
									<a-tag v-for="item in record.tagData" style="margin-bottom: 5px;" color="blue" v-if="record.tagData.length > 0 && record.tagData.length < 5">
										{{ item.tname }}
									</a-tag>
									<span v-if="(record.tagData.length > 5)">
                            <a-popover>
                                <span slot="content">
                                    <div class="over-width"> <a-tag v-for="(item) in record.tagData" style="margin-top: 5px;"   color="blue">{{item.tname}}</a-tag></div>
                                </span>
                                <span v-for="(item1,index1) in record.tagData" >
                                     <a-tag v-if="index1<5" style="margin-top: 5px;"   color="blue">{{item1.tname}}</a-tag>
                                </span>
                              <div>等{{record.tagData.length}}个标签</div>
                            </a-popover>
                        </span>
		...
		<a-modal
			title="客户查重"
			v-model="recheckingVisible" :footer="false" width="800px" @cancel="recheckingCancel">
			<a-input-search
			...
			<div style="height: 300px;margin: 15px 0;">
				<a-spin tip="加载中..." size="large" :spinning="recheckingLoading">
					<div class="spin-content">
						...
						<!--分页 -->
						<div class="pagination" style="margin: 20px 0px;float: right;"
						     v-show="total4 > 0" v-has="'highSeasCustomerList-list'">
								<span style="height: 32px;line-height: 32px;display: inline-block;vertical-align: top;margin-right: 10px;">
									共
							...
							<span class="pagination" style="height: 32px;display: inline-block;">
									<a-pagination :total="total4" showSizeChanger :showQuickJumper="quickJumper4"
									              :current="page4"
									              :pageSize="pageSize4"
									              :pageSizeOptions="['15','30','50','100']"
									              @change="changePage4"
...
export default {
	...
	methods : {
		...
		async changeRecheckingSearch (e, x, page = 1, pageSize = this.pageSize4) {
			...
			if (res.error != 0) {
				...
				this.recheckingLoading = false
				this.recheckingList = res.data.info
		...
		changePage4 (page, pageSize) {
			this.changeRecheckingSearch('', '', page, pageSize);
		},
		showSizeChange4 (page, pageSize) {
			this.changeRecheckingSearch('', '', 1, pageSize);
		},
		recheckingCancel () {
			this.recheckingLoading = false
			this.recheckingList = []
			...
			if (res.error != 0) {
				...
				if (this.assignType == 0) {
					...
					if (type == 0) {
						...
						this.addStatus = 0
						this.joinTime = null
						this.getCustomList(this.page2, this.pageSize2);
					} else if (type == 1) {
						this.batchTypeValue = false
						this.selectedRowKeys = []
						this.getCustomList(this.page2, this.pageSize2)
