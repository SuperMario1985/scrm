<template>
	<div style="width: 100%;max-height:100%;position: absolute;overflow-y: auto;padding-bottom: 30px;">
					...
					<div class="custom-info">
						...
						<div class="custom-info-text">
							...
							<div class="col">
								<span>
									...
									<span style="display: inline-block;width: calc(100% - 45px);vertical-align: text-top;">
										...
										<a-icon @click="showModal" type="edit" style="margin-left: 5px;display:none;"
							...
							<div class="col">
								...
								<a-icon type="edit" @click="showDesVisible" style="margin-left: 10px;display:none;"/>
						...
						<div class="custom-info-project single-accout-card">
							<a-steps progress-dot :current="projectList.length-1">
								<a-step v-for="item in projectList">
									...
									<div slot="description">
										<div v-if="item.status != 1">实际结束：{{item.finish_time}}
							...
							<a-tabs v-model="tabKey" type="card" @change="changeTab">
								<a-tab-pane tab="跟进记录" key="1">
									<div class="time">
										...
										<a-empty v-if="followRecord.length == 0" />
										<a-timeline class="time-line" v-if="followRecord.length > 0">
								...
								<a-tab-pane tab="用户画像" key="3">
									<div style="overflow:hidden;margin-bottom: 10px;display: none;">
		...
		<!--	打标签   -->
		<a-modal title="为客户增加标签" v-model="visible" @ok="handleOk"
			...
			<a-form-item :label-col="{ span: 3 }"
				...
				<a-select showSearch optionFilterProp="children" style="width: 180px"
				          ...
				          :dropdownStyle="{ overflow: 'hidden' }" allowClear v-model="groupId">
		...
		methods   : {
			...
			async getTags () {
				...
				if (res.error != 0) {
					...
					this.tagLoading = false
					this.disabled = []
					if (9999 - this.hasTagsValue.length <= 0) {
						...
						for (let i = 0; i < this.tagsList.length; i++) {
							let disabled = false;
							this.disabled.push(disabled);
							for (let j = 0; j < this.hasTagsValue.length; j++) {
								if (this.hasTagsValue[j] == this.tagsList[i].tagname) {
									this.disabled[i] = true
								}
							}
						}
					}
					this.tagCheckValue2 = this.tagCheckValue.concat(this.tagCheckValue2)
					this.tagCheckValue = Array.from(new Set(this.tagCheckValue2))
				}
			},
			onChange2 (checkedValues) {
				...
				let temp = [];
				let temparray = [];
				for (let i = 0; i < this.hasTags.length; i++) {
					temp[this.hasTags[i]] = typeof this.hasTags[i];

				}
				for (let i = 0; i < checkedValues.length; i++) {
					let type = typeof checkedValues[i];
				...
				if (9999 - checkedValues.length <= 0) {
					this.disabled = []
					for (let i = 0; i < this.tagsList.length; i++) {
						let disabled = true
						this.disabled.push(disabled)
			...
			handleOk () {
				if (this.tagCheckValue.length == this.hasTags.length) {
			...
			cancelVisible () {
				...
				this.tagValue = []
				this.tagCheckValue2 = []
				this.tagCheckValue = this.hasTags
			},
			//新建标签
			showAddTagModal () {
			...
			onSearch () {
				...
				if (this.inputValue == "") {
					...
					this.tagValue = []
					for (let i = 0; i < this.inputValue.split(" ").length; i++) {
