...
export default {
  ...
  methods: {
    initData(menuData) {
      ...
      this.openKeys = [
        menuList[this.$route.name] ? menuList[this.$route.name] : "",
      ];
    },
    ...
    routerRD(res) {
      ...
      if (needRepush) {
        ...
        if (that.subPagePermission == "") {
          rLink =
            res.data.menuList[this.type][0].link.length > 0
              ? res.data.menuList[this.type][0].link
              : res.data.menuList[this.type][0].children[0].link;
        } else {
          if (
            that.$store.state.permissionButton.includes(that.subPagePermission)
          ) {
            ...
            if (
              that.faterPagePermission != "" &&
              that.$store.state.permissionButton.includes(
                that.faterPagePermission
              )
            ) {
    ...
    async getMenuList() {
      ...
      if (JSON.parse(localStorage.getItem("corpArr")).length > 0) {
        this.agentCount = JSON.parse(localStorage.getItem("corpArr")).find(
          (corp) => corp.corpid == localStorage.getItem("corpId")
        ).agent_count;
      } else {
        this.agentCount = "0";
      }
      ...
      if (res.error != 0) {
        ...
        if (
          res.data.menuList[0].length == 0 &&
          res.data.menuList[1].length == 0
        ) {
          ...
          if (localStorage.getItem("isMasterAccount") == 2) {
            Promise.all([that.$store.dispatch("getPermissionButton")])
              .then((resGetPBtn) => {
                if (
                  that.subPagePermission != "" &&
                  that.$store.state.permissionButton.includes(
                    that.subPagePermission
                  )
                ) {
