    ...
    >
      <a-spin tip="Loading..." size="large" :spinning="isLoading2">
        ...
        >
          ...
          <a-input-search
            style="margin-bottom: 8px; width: 75%"
            placeholder="搜索成员、部门"
            @change="onChange"
            v-model="name"
          ...
          <div v-if="searchVisible" style="padding: 10px 10px 10px 0">
            ...
            <template
              v-if="
                searchDepartmentList.workUser &&
                searchDepartmentList.workUser.length > 0
              "
            >
              ...
              <template v-for="item in searchDepartmentList.workUser">
                <a-popover placement="top" :overlayStyle="{ 'z-index': 99999 }">
                  <span slot="content" v-html="item.departmentNameAll"> </span>
                  <div
                    class="content-line"
                    @click="
                      item.disabled
            ...
            <template
              v-if="
                searchDepartmentList.department &&
                searchDepartmentList.department.length > 0
              "
            >
              <div class="title-type">部门</div>
              <template v-for="item in searchDepartmentList.department">
                <a-popover placement="top" :overlayStyle="{ 'z-index': 99999 }">
                  <span slot="content" v-html="item.titleAll"> </span>
                  <div
                  ...
                  >
                    <div style="white-space: nowrap">
                      <a-icon
                        type="apartment"
                        style="margin-right: 10px; color: #01b065"
                      />
                      {{ item.title }}
                      <a-icon
                        type="check"
  ...
  methods: {
    ...
    async refreshFans() {
      ...
      const { data: res } = await this.axios.post(
        "work-msg-audit/get-permit-user-list",
        {
          corp_id: this.corpId,
          refresh: 1,
        }
    ...
    onChange(e) {
      if (e.target.value != "") {
        ...
        if (
          typeof this.departmentList[0] != "undefined" &&
          typeof this.departmentList[0].children != "undefined" &&
          typeof this.departmentList[0].children[0] != "undefined" &&
          typeof this.departmentList[0].children[0].key != "undefined"
        ) {
    ...
    async onLoadData(treeNode) {
      return new Promise((resolve) => {
        ...
        Promise.all([this.getDepartmentList(treeNode.dataRef.department_id)])
          .then((res) => {
