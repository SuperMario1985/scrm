			...
			<a-spin tip="加载中..." size="large" :spinning="isLoading2">
				<div style="width: 50%;height: 450px;float: left;border-right:1px solid #D9D9D9;overflow: auto;">
					<p v-if="noticeTitle != '' || isAudit == 1" style="margin-bottom: 10px;">
						...
						<span v-if="isAudit == 1 || isAudit == 2" style="color: #01b065;cursor: pointer;"
						      @click="confirm">
							<a-icon type="redo" v-if="!loading"/><a-icon v-if="loading" type="sync"
							                                             style="color: #01b065;" spin/>同步成员</span>
					</p>
					<div style="margin-bottom: 8px;">
						<a-input-search style="width: 75%;" placeholder="搜索成员、部门"
						                @change="onChange" v-model="name" @pressEnter="searchEnter"/>
						<span v-if="is_external == 1 || is_from == 7" style="color: #01b065;cursor: pointer;display: inline-block;width: 25%;text-align: center;" @click="refresh">
							<a-icon type="redo" v-if="!loading2"/><a-icon v-if="loading2" type="sync"
							                                              style="color: #01b065;" spin/>同步成员
						</span>
					</div>
								...
								<a-popover placement="top" :overlayStyle="{'z-index':99999}">
									...
									<div class="content-line"
										...
										<div class="title-left">
											...
											{{ item.title }}
											<a-icon type="check" style="margin-left: 10px;color: #01b065;"
								...
								<a-popover placement="top" :overlayStyle="{'z-index':99999}">
									...
									<div style="padding: 10px 0;"
										...
										<div style="white-space: nowrap;">
											<a-icon type="apartment" style="margin-right: 10px;color: #01b065;"/>
											{{ item.title }}
											<a-icon type="check" style="margin-left: 10px;color: #01b065;"
											        v-if="rightIdList.indexOf(item.id) > -1"/>
										</div>
					...
					>
						...
						<div class='w-full' slot="custom" slot-scope="{title,key,id,scopedSlots,isLeaf,disabled,ids}"
						     @click="disabled ? '' : onCheck(title,key,id,scopedSlots,isLeaf,ids)">
							<div class="flex items-center justify-between">
								...
								<span v-if='sopArr.indexOf(id) > -1'>(已有规则)</span>
							</div>
						</div>
					</a-tree>
				...
				<div style="width: 50%;height: 450px;float: left;padding-left: 20px;overflow: auto;">
					...
					<a-list itemLayout="horizontal" :dataSource="rightList">
						<a-list-item slot="renderItem" slot-scope="item, index">
							...
							<a-icon type="close-circle" style="font-size: 16px; margin-right: 10px;"
...
export default {
	...
	data () {
		return {
			...
			loading       : false,//同步会话存档成员按钮的loading
			loading2      : false,//同步具有外部联系人权限的成员按钮的loading
	...
	methods: {
		...
		confirm () {
			...
			this.loading = true
			this.$message.destroy()
			this.$message.info("已进入后台刷新中，请耐心等待...", 2,this.refreshFans)
