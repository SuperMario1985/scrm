		...
		<template v-has="'channel-group-list'">
			<div class="team-add" style="margin-top: 80px;">
				...
				<span style="color: #1890FF;margin-left: 5px; cursor: pointer;position:absolute;margin-right: 5px;"
				      ...
				      @click="addGroupModal('')" v-has="'channel-group-add'"><a-icon class="team-add-icon" type="plus"
						...
						>
							<template slot="content">
								<div @mouseover="clearLeave" @mouseleave="hideMoreIcon1">
									<p class="operation" @click="addGroupModal(key)" v-has="'channel-group-child-add'">
										创建子分组</p>
									...
									<p class="operation2" v-if="delFlag" @click="removeGroup(key, title)"
									   v-has="'channel-group-del'">删除分组</p>
									<p class="operation1" v-if="!delFlag" v-has="'channel-group-del'">删除分组</p>
								</div>
							</template>
	...
	export default {
		...
		created () {
			if(localStorage.getItem('permissionButton') == 'all'){
				...
				if(permissionButton.indexOf('channel-group-child-add') == -1 && permissionButton.indexOf('channel-group-edit') == -1 && permissionButton.indexOf('channel-group-del') == -1){
		...
		methods: {
			...
			removeGroup (key, title) {
				...
				let that = this;
				that.$confirm({
					title     : "【" + title + "】一旦删除，归属于该分组的渠道活码都将被移至【未分组】，确定删除分组吗?",
					okText    : "确定",
					okType    : "primary",
					cancelText: "取消",
					onOk () {
			...
			async delGroup (key) {
				const {data: res} = await this.axios.post("work-contact-way/group-del", {
			...
			async addGroup () {
				...
				const {data: res} = await this.axios.post("work-contact-way/group-add", {
			...
			async moveGroup (parentId, groupIdList, data) {
				const {data: res} = await this.axios.post("work-contact-way/group-sort", {
					uid      : localStorage.getItem('uid'),
					parent_id: parentId,
