<template>
	<div v-if="msgObj && msgObj.typeValue">
		...
		<template v-if="msgObj.typeValue && msgObj.typeValue == 5"
			...
			<a-form-item v-if="typeof msgObj.isSketchAdd == 'undefined' || msgObj.isSketchAdd" label="添加方式"
			             :label-col="{ span: 3 }"
			             :wrapper-col="{ span: 21 }">
...
<!--				<span style="vertical-align: bottom;">（图片大小不超过2M，支持JPG、JPEG及PNG格式）</span>-->
		...
		<template v-if="msgObj.typeValue && msgObj.typeValue == 1">
			<!-- 文本消息 -->
			<a-form-item v-if="typeof msgObj.isTextAdd != 'undefined' && msgObj.isTextAdd == false" label="添加方式"
			             :label-col="{ span: 3 }"
			             :wrapper-col="{ span: 21 }">
			...
			<a-form-item label="文本消息" :label-col="{ span: 3 }"
			             :wrapper-col="{ span: 21 }">
				<editor v-if="msgObj.addType == 1" :text="msgObj.textContent" :wordLimit="wordLimit"
		...
		<template v-if="msgObj.typeValue && msgObj.typeValue == 6">
			<a-form-item v-if="typeof msgObj.isAppletAdd == 'undefined' || msgObj.isAppletAdd" label="添加方式"
			             :label-col="{ span: 3 }"
			             :wrapper-col="{ span: 21 }">
			...
			<template v-if="msgObj.addType == 1">
				<div class="content-msg" style="margin-bottom: 10px;">
					<p style="margin-bottom: 2px;line-height: 20px;">
						在
						<template v-if="menuType == 1">企业微信</template>
						<template v-else>公众号</template>
						里发送小程序（必须在微信公众平台通过审核和发布的），请先将其关联到
						<template v-if="menuType == 1">企业微信</template>
						<template v-else>公众号</template>
						（
						<a v-if="menuType == 1" target="_blank"
						   href="https://support.qq.com/products/104790/faqs/61672">
							如何关联</a>
						<a v-if="menuType == 0" target="_blank"
						   href="https://support.qq.com/products/104790/faqs/90038">
							如何关联</a>
...
<!--					<span style="display: inline-block; line-height: 22px;">（上传图片不超过1M，尺寸必须为1080*864像素，支持jpg/jpeg/png格式）</span>-->
				...
				<a-form-item :label-col="{ span: 4 }"
					...
					<a-input v-model="msgObj.appid" placeholder="请填写小程序AppID，必须是关联到企业的小程序应用"/>
				</a-form-item>
				<!-- page路径 -->
				<a-form-item :label-col="{ span: 4 }"
				             :wrapper-col="{ span: 20 }">
					...
					<a-input style="margin-bottom: 10px;"
					         v-model="msgObj.pageUrl" placeholder="请填写小程序AppID，必须是关联到企业的小程序应用">
					</a-input>
				</a-form-item>
				<a-form-item label="素材同步" :label-col="{ span: 4 }"
				             :wrapper-col="{ span: 20 }">
    ...
    computed: {
      ...mapGetters({
		...
		methods   : {
		  //保存菜单
      saveMenu(){
        this.$emit('saveMenu',this.msgObj )
      },
			//更换消息类型
			changeTypeValue () {
			...
			async getOneMaterial (id) {
				...
				if (res.error == 0) {
					...
					if (this.msgObj.typeValue == 5 && this.msgObj.sketchList[this.currentSketch].addType == 1) {
						...
						this.msgObj.sketchList[this.currentSketch].closeShowModal3 = true
						this.msgObj.sketchList[this.currentSketch].local_path.img = this.commonUrl + materialList.artList[0].local_path
