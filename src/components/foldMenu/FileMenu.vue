    ...
    >
      ...
      >
        <div style="min-width: 100%" @mouseover="showMoreIcon(key)">
          <div style="display: inline-block" :class="key == id ? 'active' : ''">
            {{ title }}
          </div>
            ...
            <template slot="content">
              <div @mouseover="clearLeave" @mouseleave="hideMoreIcon1">
                ...
                <p
                  class="operation"
                  v-if="
                    is_move_type != 1 && is_move_type != 2 && is_move_type != 3
                  "
                  @click="editGroupName(key, title)"
                  v-has="'material-group-edit'"
                >
                  编辑分组
                </p>
                <p
                  class="operation2"
                  v-if="delFlag && is_move_type != 2 && is_move_type != 3"
                  @click="removeGroup(key, title, channel_type)"
                  v-has="'material-group-delete'"
                >
                  删除分组
                </p>
                <p
...
export default {
  ...
  created() {
    if (localStorage.getItem("permissionButton") == "all") {
      ...
      let permissionButton = JSON.stringify(
        localStorage.getItem("permissionButton")
      ).split(",");
      if (
        permissionButton.indexOf("material-group-add") == -1 &&
        permissionButton.indexOf("material-group-edit") == -1 &&
        permissionButton.indexOf("material-group-delete") == -1
      ) {
    ...
    removeGroup(key, title, channel_type) {
      ...
      that.$confirm({
        title:
          "【" +
          title +
          "】一旦删除，归属于该分组的资料都将被移至【未分组】，确定删除分组吗?",
        okText: "确定",
        okType: "primary",
        cancelText: "取消",
        onOk() {
