<template>
	<div class="group-tree1" style="position: absolute; width: 100%; height: 100%; overflow: auto;">
		<div style="position: absolute;" :style="{'left': getClientWidth + 'px'}" class="sider-one-txt">选择企业微信</div>
		<a-select
				showSearch
		...
		<template v-has="'groupAutoCreate-group'">
			...
			>
				<template slot="custom" slot-scope="{title,key, num, is_not_group}">
					<div style="min-width: 100%;" @mouseover="showMoreIcon(key)">
						...
						<a-popover v-if="key == mouseOverId && is_not_group != 1 && isShowPop"
						           placement="right"
						           trigger="hover"
						>
							<template slot="content">
								<div @mouseover="clearLeave" @mouseleave="hideMoreIcon1">
									<p class="operation" @click="addGroupModal(key)"
									   v-has="'groupAutoCreate-group-add'">
										创建子分组</p>
									<p class="operation" @click="editGroupName(key, title)"
									   v-has="'groupAutoCreate-group-edit'">
										编辑分组</p>
									<p class="operation2" v-if="delFlag" @click="removeGroup(key, title)"
									   v-has="'groupAutoCreate-group-del'">删除分组</p>
									<p class="operation1" v-if="!delFlag" v-has="'groupAutoCreate-group-del'">删除分组</p>
								</div>
							</template>
	...
	export default {
		...
		created () {
			if (localStorage.getItem('permissionButton') == 'all') {
				...
				if (permissionButton.indexOf('groupAutoCreate-group-add') == -1 && permissionButton.indexOf('groupAutoCreate-group-edit') == -1 && permissionButton.indexOf('groupAutoCreate-group-del') == -1) {
		...
		methods: {
			...
			clickAll () {
				this.id = 0
				this.$emit("changeGroupId", this.gData, [])
			},
			// 分组操作
			lookOperation (key) {
				let that = this
			...
			removeGroup (key, title) {
				...
				let that = this;
				that.$confirm({
					title     : "【" + title + "】一旦删除，归属于该分组的信息都将被移至【未分组】，确定删除分组吗?",
					okText    : "确定",
					okType    : "primary",
					cancelText: "取消",
					onOk () {
			...
			async delGroup (key) {
				const {data: res} = await this.axios.post("work-chat-way/group-del", {
			...
			async addGroup () {
				...
				const {data: res} = await this.axios.post("work-chat-way/group-add", {
			...
			getChildrenId (key) {
				...
				this.$emit("changeGroupId", this.gData, ids)
			},
			// 获取分组
			async getGroupList () {
			...
			async moveGroup (parentId, groupIdList, data) {
				const {data: res} = await this.axios.post("work-chat-way/group-sort", {
					uid      : localStorage.getItem('uid'),
					parent_id: parentId,
